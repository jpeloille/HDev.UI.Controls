...
/// testé) et DocLayoutEngine (même rendu que HDevHtmlView).
/// Hauteur au contenu : placer dans un ScrollViewer. Html en entrée/sortie.
/// </summary>
public class HDevRichEdit : Control
{
    ...
    static HDevRichEdit()
    {
        FocusableProperty.OverrideDefaultValue<HDevRichEdit>(true);
    }
    ...
    private void ReplaceEditor(HDevDocument document)
    {
        _editor.Changed -= OnEditorChanged;
    ...
    public async Task InsertLinkInteractiveAsync()
    {
        ...
        var href = await HDevInputBox.ShowAsync(owner,
    ...
    private async Task CopyAsync()
    {
        ...
        // Texte brut + HTML : un autre HDevRichEdit (ou LibreOffice…) recolle riche
    ...
    public override void Render(DrawingContext context)
    {
        ...
        context.FillRectangle(new SolidColorBrush(HDevTheme.Background.Control), bounds);
        ...
        if (_caretVisible && IsFocused && !_editor.HasSelection)
        {
            var rect = CaretRect();
            if (rect != null)
                context.FillRectangle(new SolidColorBrush(HDevTheme.Text.Primary), rect.Value);
        }
    }
}
