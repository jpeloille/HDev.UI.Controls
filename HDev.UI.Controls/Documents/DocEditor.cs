...
{
    private readonly List<(HDevDocument Doc, DocCaret Caret)> _undo = new();
    private readonly List<(HDevDocument Doc, DocCaret Caret)> _redo = new();
    private bool _lastOpWasTyping;
    private const int UndoLimit = 100;

    public HDevDocument Document { get; private set; }
    ...
    public DocEditor(HDevDocument? document = null)
    {
        Document = document ?? new HDevDocument();
        Normalize(Document);
        Reflatten();
    ...
    private static void Normalize(HDevDocument document)
    {
    ...
    private void Restore(HDevDocument doc, DocCaret caret)
    {
        Document = doc;
        Reflatten();
        Caret = ClampCaret(caret);
...
{
    public static HDevDocument Clone(HDevDocument source)
    {
        var clone = new HDevDocument { BaseStyle = source.BaseStyle };
...
/// <summary>HDevDocument → HTML (sortie du HDevRichEdit, envoi de mails)</summary>
public static class HtmlSerializer
{
    ...
    public static string Serialize(HDevDocument document)
    {
        var sb = new StringBuilder();
