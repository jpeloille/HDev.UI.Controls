...
Côté HDev.UI.Controls, la même pile sert trois canaux avec trois listes distinctes :

- **vocabulaire ACCEPTÉ** (parseur tolérant, profil `Mail`) — inchangé, pour le
  lecteur de mails (HDevHtmlView) et le collage riche ;
...
- **vocabulaire ÉMIS** (`HtmlSerializer`) — sous-ensemble strict du précédent :
  la sortie de `HDevRichEdit.Html` est toujours du storage-v1 propre.
