# HDev.UI.Controls — Contexte du projet
...
- **Thème centralisé** : `HDevTheme.cs` contient toutes les constantes (couleurs, tailles, polices)
...
- **Dépendances** : Avalonia UI 11.2.1 + projet frère `HDev.UI.DataGrid` (datagrid maison, ../HDev.UI.DataGrid)
...
|----------|----------------------|------|--------------|
| HDevButton | SimpleButton | ✅ | pas de variantes DropDown/Split |
| HDevCheckBox | CheckEdit | ✅ | — (tri-state réel) |
| HDevRadioButton | RadioGroup | ✅ | — (exclusivité + flèches OK) |
| HDevToggleSwitch | ToggleSwitch | ✅ | pas d'animation du thumb |
| HDevSlider | TrackBarControl | ✅ | horizontal seulement, pas de range |
| HDevProgressBar | ProgressBar + Marquee | ✅ | — |
| HDevBadge | Badge | ✅ | Info = Primary (même couleur) |
| HDevTextBox | TextEdit | 🟡 | **ni masque, ni validation** (HasError = flag externe). Multiline OK (`IsMultiline`, lot 1) |
| HDevCard | GroupControl | ✅ | `Elevation` appliquée + `Title`/`HasBorder` réactifs (lot 1) |
| HDevWindow | XtraForm | 🟡 | chrome custom = chemin secondaire (natif par défaut, cf. X11) ; Light forcé, pas de dark ; champs restore morts |
| HDevMenuBar / HDevMenuItem / HDevContextMenu | BarManager, PopupMenu | ✅ | raccourcis RÉELS (HDevShortcutManager, 14/07), Alt+lettre ouvre les menus, navigation ↑↓/Enter/←→/Échap (sous-menus compris), `IsEnabled` bloque le clic |
| HDevRibbon | RibbonControl | 🟡 | rendu = boutons + séparateurs + dialog launcher (lot 1). Classes inertes (ComboBox/Gallery/ToggleGroup) retirées ; `AccentColor`/`IsContextual` toujours non exploités ; pas de backstage/QAT |
| HDevComboBox | ComboBoxEdit | ✅ | éditable (lot 2) : `TextEditStyle` Standard/DisableTextEditor/HideTextEditor, `AutoComplete` Suggest/Append/SuggestAppend, `ImmediatePopup`, `SelectAll`, texte libre accepté. `DrawItem` (owner-draw) toujours absent |
| HDevEditorBase (socle) | BaseEdit/RepositoryItem | ✅ | masque simple (0/9/L/A + littéraux, suppression = troncature alignée), `Validating` annulable au blur, erreur bordure rouge + tooltip (lot 2) |
| HDevDateEdit / HDevTimeEdit | DateEdit / TimeEdit | ✅ | masque format, calendrier popup (recréé à chaque ouverture — état périmé sinon), MinDate/MaxDate, spin ±min/±h (lot 2) |
| HDevSpinEdit | SpinEdit | ✅ | Increment/bornes/décimales, spin+molette+flèches, filtre numérique culture (lot 2). CalcEdit (calculette popup) → backlog |
| HDevLookUpEdit | LookUpEdit/GridLookUpEdit | 🟡 | dropdown HDevDataGrid multi-colonnes, DisplayMember/ValueMember, Suppr efface (lot 2). Pas de saisie/filtre texte ni flip haut/bas |
| HDevToolbar | Bars | 🟡 | boutons icône/texte, toggles, séparateurs (lot 1). Pas d'overflow, pas de drag |
| HDevStatusBar | RibbonStatusBar | ✅ | panneaux gauche/droite, séparateurs, panneaux cliquables (lot 1) |
| HDevMessageBox | XtraMessageBox | ✅ | modale OK/OKCancel/YesNo/YesNoCancel, icônes, résultat typé, ShowAsync + raccourcis (lot 1) |
| HDevDataGrid (réf. `../HDev.UI.DataGrid`, source de vérité) | GridControl/GridView | 🟡 | voir section HDevDataGrid ci-dessous |
| HDevTabControl | XtraTabControl | ✅ | onglets pilule, contenu hébergé, fermeture optionnelle (TabClosing annulable, clic milieu), clavier ←→ (lot 4a). Pas d'overflow d'onglets |
| HDevAccordion | AccordionControl/NavBarControl | ✅ | sections empilées à contenu hébergé, chevron, `ExpandMode` Single (exclusif) / Multiple, `SectionExpanding` annulable, clavier ↑↓/Home/End/Enter/Espace, `HeaderBackground` par section (états vert/rouge, hover en surcouche) (14/07, validé). Pas d'animation, pas de sous-niveaux |
| HDevAvatar | — (pas d'équivalent DX direct) | ✅ | initiales dérivées de `FullName` (pas `Name` : réservé Avalonia) ou posées, photo rognée cercle, tailles S/M/L, pastille présence, couleur stable par hash FNV du nom (14/07, validé) |
| HDevChip | — (Tile/Tag) | ✅ | pilule cliquable/cochable (visuel accent 30/90 aligné TokenEdit)/fermable (`CloseRequested` annulable, `Closed` = à l'app de retirer), badge compteur (`Badge`/`BadgeColor`), sur HDevControlBase (états+clavier gratuits) (14/07, validé) |
| HDevToggleButtonGroup | — (segmented, barre Jour/Semaine/Mois) | ✅ | segments dans une pilule, Single exclusif (flèches changent la sélection) / Multiple (flèches déplacent le focus, Espace bascule), texte inverse sur accent (14/07, validé) |
| HDevTreeView | TreeView | ✅ | expand/collapse, sélection, clavier complet (↑↓←→ Enter Home/End), événements, ExpandAll/CollapseAll (lot 4a). Non virtualisé, pas de checkboxes/édition |

### 4bis. HDevDataGrid — état (audit du 10/07/2026)
...
Note : le README de HDev.UI.DataGrid est marketing (tout ✅), se fier au code / à cet audit.

### 4ter. Manques vs DevExpress WinForms, priorisés pour Synaxis/Ovidie

**Critique (bloquant apps métier)** :
- HDevDateEdit / HDevTimeEdit (FTL, planning, échéances) — l'éditeur le plus utilisé
- HDevSpinEdit / HDevCalcEdit (numérique formaté — cœur d'Ovidie)
- Socle commun masques + validation (prérequis des deux ci-dessus, trou de HDevTextBox)
- HDevTabControl
- HDevLookUpEdit / GridLookUpEdit (référentiels avions/équipages/comptes ; s'appuyer sur HDevDataGrid)
- HDevMessageBox / HDevDialog (XtraMessageBox) — trivial, utilisé partout

**Important (structure d'application)** : HDevTreeView puis TreeList (arbre+colonnes), HDevToolbar, HDevStatusBar, HDevSearchControl, MemoEdit (multiline via HDevTextBox), AlertControl/Toast, WaitForm/Splash, Docking (gros chantier, poste dispatcher — phase 1 ✅), NavBar ✅/Accordion ✅ (14/07), ButtonEdit, TokenEdit ✅.
...
1. ~~Quick wins : HDevMessageBox, HDevStatusBar, HDevToolbar, multiline HDevTextBox + assainir les façades~~ ✅ **Fait le 10/07/2026** (validé visuellement)
2. ~~Éditeurs : socle masque+validation → HDevDateEdit → HDevSpinEdit → HDevComboBox éditable/autocomplete → HDevLookUpEdit~~ ✅ **Fait le 14/07/2026** (benchmark masque 15/15, corrigé et validé visuellement ; restes : CalcEdit, saisie/filtre texte du LookUp, flip haut/bas des popups éditeurs)
3. ~~HDevDataGrid : brancher l'UI sur le moteur existant (opérateurs de filtre, Shift+clic, pin→freeze, best-fit, sélection cellule, copier/coller, export CSV)~~ ✅ **Fait le 14/07/2026** (commit HDev.UI.DataGrid f430933 ; restes : coller, réapplication des états cellule au recyclage, vitrine du mode Cell)
4. Gros chantiers :
   - ~~HDevTabControl/TreeView~~ ✅ **Lot 4a fait le 14/07/2026** (validé visuellement)
   - **Arbitrages actés (14/07/2026)** :
     - **HDevGantt maison, spécialisé GESTION DE PROJET** (tâches WBS, jalons, dépendances FS/SS/FF/SF, avancement, chemin critique CPM) — PAS un planning de ressources. Architecture : moteur d'ordonnancement pur testé (roll-up, calendrier ouvré, propagation, CPM) + UI custom-rendered (table arbre à gauche façon MS Project, timeline à droite). Phase 1 = afficher, phase 2 = ordonnancer/interagir, phase 3 = baseline/export.
     - **HDevGantt phase 1 ✅ faite le 14/07/2026** (validée) : moteur GanttProject/GanttTask/GanttCalendar/GanttTimeAxis (24 tests), contrôle virtualisé lignes×temps (>2000 tâches fluide), table arbre + timeline, barres/avancement, crochets récapitulatifs, jalons losanges, flèches de dépendances routage MS Project (L + S à rebours), ombrage chômés, now-line, zoom Ctrl+molette centré, splitter, tooltips. Calibrage : dates posées par l'app, calendrier ouvré v1, cible >2000 tâches.
     - **HDevGantt phase 2a ✅ faite le 14/07/2026** (validée) : GanttScheduler — ordonnancement auto (`Project.AutoSchedule`, défaut off ; tri topologique Kahn, calage FS/SS/FF/SF + lag en jours ouvrés, durée préservée, `IsManuallyScheduled` exclut), chemin critique par dates (marge totale ouvrée `TotalFloatDays`, `IsCritical`, récapitulatives héritent), cycles refusés par `DependsOn` (`WouldCreateCycle`). 19 tests. UI : `HighlightCriticalPath` (barres rouges) + toggles démo.
     - **HDevGantt phase 2b ✅ faite le 14/07/2026** (validée) : interactions souris — déplacement (fantôme pointillé + étiquette, snap jour ouvré, durée ouvrée préservée), resize par les bords, poignée d'avancement (triangle sous la barre, snap 5 %), création de liens (connecteur + ligne élastique verte/rouge, cycle refusé), Échap annule, `IsReadOnly`. Contrat « le contrôle affiche, le métier décide » : `TaskDatesChanging`/`ProgressChanging`/`LinkCreating` annulables (+ événements Changed/Created). Aucune mutation pendant le drag (commit au relâcher, un seul Recalculate).
     - **HDevGantt phase 3 ✅ faite le 14/07/2026** (validée, dont l'export hors écran) : baseline (`SetBaseline`/`ClearBaseline` instantané des dates effectives, barres grises prévues + carrés creux jalons, écart ouvré dans le tooltip, toggle `ShowBaseline`), édition in-place de la table (double-clic/F2 → HDevTextBox/HDevDateEdit/HDevSpinEdit en dogfooding, Enter/Échap, mêmes événements annulables que le drag), clic droit → HDevContextMenu (suppression de liens via `LinkRemoving`/`Removed` annulables, bascule ordonnancement manuel), `ZoomToFit()` + `ExportPng(path, entireProject)` (rendu hors écran du projet complet). **HDevGantt terminé sur les 3 phases prévues.** Prochain différenciant : intégration LiveCharts2 (Ovidie).
     - **HDevGantt « app-ready » ✅ fait le 14/07/2026** (build propre, +12 tests, à valider visuellement — boutons dans la page Projet) — demandé pour bâtir une app de gestion de projet pur (activités RDOV) :
       ...
       - **Undo/redo** : `GanttHistory` pur testable (piles de snapshots JSON, plafond 50, ignore doublon en sommet). HDevGantt snapshote AVANT chaque mutation réelle (drag dates/avancement/lien, édition table nom/dates/%, suppression de lien, bascule manuel — tous après le check `Cancel`), `Undo()`/`Redo()`/`CanUndo`/`CanRedo`/`HistoryChanged`/`ClearHistory`, **Ctrl+Z / Ctrl+Y (Ctrl+Maj+Z)**, sélection re-résolue par Id après restauration. Nouveau projet (setter `Project`) purge l'historique.
       - **Restes app** : colonnes de table figées (nom/début/fin/%), pas d'impression paginée (seul `ExportPng`), `Tag` non sérialisé (utiliser `Id`), pas de binding MVVM (graphe construit impérativement). **→ voir la section « HDevGantt → outil pro : feuille de route HDev.UI.Controls » (P0-P3) et `UGantt-charte.md`.**
     - **Charts Ovidie : LiveCharts2** (MIT, Avalonia natif, forkable) avec habillage HDevTheme — pas de développement maison.

**Tests unitaires du socle (14/07/2026)** : `HDev.UI.Controls.Tests` (HDevMaskEngine + moteurs Gantt/Document/Scheduler/MindMap/Dock + sérialisation Gantt + GanttHistory, 206 tests) et `HDev.UI.DataGrid/tests` (GridFilter/GridDataSource/PropertyAccessor/FormatValue, 61 tests) — `dotnet test` sur chaque projet. À maintenir : toute évolution d'un moteur pur passe par là.

## HDevMindMap — carte mentale de brainstorming ✅ fait le 14/07/2026 (validé)
...
- **Grammaire d'édition** (souris seule, clavier seul, ou mixte) : Tab = enfant, Enter = frère (édition immédiate HDevTextBox in-place), Suppr, F2/double-clic, Espace = repli ; **pastille plier/déplier cliquable** sur le connecteur (compte de descendants repliés) ; badges de coin du nœud sélectionné : **« + » bas-extérieur** (ajout) et **« × » haut-extérieur** (suppression, racine exclue) — hors du couloir des connecteurs ; **clic droit = HDevContextMenu** complet ; drag d'un nœud = re-parentage (élastique vert/rouge, descendants interdits, `NodeReparenting` annulable) ; navigation flèches transposée par mode ; undo/redo instantanés ; `ZoomToFit`/`CenterOnRoot`/`ExportPng` (hors écran). Dark mode suivi.
- Restes v2 : sérialisation (JSON + FreeMind .mm), notes/icônes par nœud, liens transverses, multi-sélection.

## HDevGantt → outil pro : feuille de route HDev.UI.Controls (posée le 14/07/2026)

Contexte : projet **UGantt** (logiciel de gestion de projet pur, activités RDOV) à bâtir sur HDevGantt.
...
migre dans HDev.UI.Controls quand un **2ᵉ consommateur existe**. Couvrir Cat 1 **ne fait PAS remonter les
ressources dans HDev.UI.Controls** : un moteur PM complet (nivellement, contours de charge, EVM) est du
**métier** (« le contrôle affiche, le métier décide ») → il vit dans **`UGantt.Core`**, couche domaine
pure et testée. HDev.UI.Controls ne gagne que des **primitives de rendu + les points d'extension**.
D'où : ressources/coûts/EVM et interop MSP XML → **UGantt.Core/UGantt** ; impression paginée →
**HDev.UI.Controls** (seule exception : manque dans TOUTE la suite, capacité transverse) ; PERT/réseau et
grille échelonnée → HDev.UI.Controls (rendu), leurs données → UGantt.Core.

**⚑ Le constat central (audit du 14/07/2026)** : HDevGantt savait tout afficher et tout modifier,
...
### La part HDev.UI.Controls du programme (le plan complet fait foi dans `UGantt-charte.md` §7)
...
> feature d'UGantt force une réécriture de HDev.UI.Controls.
>
...
> grille d'usage (L18) de HDev.UI.Controls. Tous les clones de Gantt l'esquivent — c'est ce qui les
...
**Réutilisable tel quel** : Ribbon (basique), menus/contextuels, toolbar, statusbar, TabControl, TreeView (volet dossiers), HDevDataGrid (tâches, vues tabulaires), toute la famille éditeurs (formulaires contact/RDV), MessageBox, badges, HDevWindow.

**🔴 Les 4 chantiers structurants** :
1. ~~**Rendu HTML des mails**~~ ✅ **v1 faite le 14/07/2026** (« les deux monstres », validée) : architecture « un socle, deux têtes » — HDevDocument (modèle riche) + HtmlParser (sous-ensemble sanitisé PAR CONSTRUCTION : script/iframe/handlers éliminés, aucun réseau, images data: décodées / distantes via ImageResolver applicatif) + DocLayoutEngine (TextLayout Avalonia, tables simples, listes, citations, code, images, virtualisation) + **HDevHtmlView** (liens cliquables, re-wrap). **v2 faite le 14/07/2026** (à valider visuellement) : **sélection au drag + copie** dans le viewer (position = DocTextBox du layout + offset, surlignage HitTestTextRange, Ctrl+C/Ctrl+A/Échap, curseur I-beam sur le texte, sélection purgée au changement de Html) ; le CSS inline était déjà large (couleurs, fond, tailles, familles, text-align, font-weight). Restes v3 : images inline dans le flux texte (v1 = blocs), copie HTML de la sélection viewer.
2. ~~**HDevRichEdit**~~ ✅ **v1 faite le 14/07/2026** (validée) : DocEditor (cœur pur testé — positions, insertion/suppression, découpe de runs pour style par plage, scission/fusion de paragraphes, undo/redo par instantanés à frappes regroupées, liens normalisés en runs LinkHref) + HtmlSerializer (HTML échappé en sortie) + **HDevRichEdit** (caret clignotant par TextLayout, sélection souris/clavier, Ctrl+B/I/U/Z/Y/A/C/X/V, Enter/Backspace, titres, barre d'outils). **v2 faite le 14/07/2026** (moteur 20 tests ; UI à valider visuellement) : **listes** (puces/numérotées — wrap/unwrap/conversion, Enter = nouvel item, Enter sur item vide = sortie, Backspace en tête = retire la puce avec scission de liste, idiome Word), **alignement** (4 modes + justify sérialisé), **couleurs/surlignage/taille** (pose ET effacement via TransformSelection — Merge ne sait pas effacer), **liens** (SetLink/GetCurrentLink, **Ctrl+K** → HDevInputBox, nouveau contrôle réutilisable), **collage HTML riche** (fragment 1 paragraphe fondu inline, multi-blocs scindé au caret, 1 undo) + **copie riche** (DataObject texte + text/html ; GetSelectedHtml par clone+trim = structure préservée), **insertion d'images** (fichier → data: base64, HTML portable). Raccourcis : Ctrl+K/Ctrl+Maj+L/Ctrl+E/Ctrl+Maj+R/Ctrl+J. Restes v3 : édition de tables, images inline dans le flux (blocs en v1), styles de listes imbriquées (Tab).
3. ~~**HDevScheduler**~~ ✅ **v1 faite le 14/07/2026** (validée) : moteur pur testé (expansion des récurrences None/Daily/Weekly-jours/Monthly + Interval/Until, couloirs de chevauchement greedy par clusters, plages de vues) + contrôle — vues Jour/Semaine ouvrée/Semaine (grille horaire scrollable, heures non ouvrées et week-ends ombrés, bandeau toute-la-journée, ligne « maintenant », événements côte à côte) et Mois (chips + « +n autres », aujourd'hui en pastille, molette = mois). Drag déplacement + resize (fantôme, snap 15 min, `EventChanging` annulable, récurrents non déplaçables v1), double-clic créneau → `TimeSlotDoubleClicked`, navigation ◀ Aujourd'hui ▶. Restes v2 : édition/détachement d'occurrences récurrentes, drag multi-jours, catégories/légendes, fuseaux.
4. ~~**HDevListView**~~ ✅ **v1 faite le 14/07/2026** (validée) : liste virtualisée à items riches — ItemAdapter structuré (Titre/Aperçu/Trailing/Icône initiales/Badge/Emphasized = non-lu gras + barre accent), GroupSelector → en-têtes repliables avec compte, actions au survol (la date s'efface), sélection Single/Multi (Ctrl/Shift/Ctrl+A), clavier complet, ObservableCollection réactive, ellipsis. Démo « Boîte » = mini-Outlook 2 volets (HDevListView + HDevHtmlView, ouvrir = lu, suppression réactive). Restes v2 : drag & drop vers dossiers, hauteur d'item variable, avatars image.

**⚑ HTML TipTap = format de stockage (acté le 19/07/2026)** : le HTML `getHTML()` de TipTap (sérialisation fermée d'un document ProseMirror) devient le format de stockage du texte riche applicatif. Vocabulaire figé **storage-v1** : source de vérité `Documents/HtmlVocabulary.cs` + miroir humain `Documents/StorageHtml-v1.md` (toute divergence = bug). Trois listes distinctes : accepté (profil Mail tolérant, inchangé — lecteur de mails + collage) ⊃ strict (storage-v1, `Parse(html, HtmlParseOptions.Storage)` = diagnostics jamais d'exception) ⊇ émis (`HtmlSerializer`, dialecte TipTap : strong/em, `<mark>`, `<pre><code>`, tbody, target/rel sur les liens). ✅ **Fait le 19/07/2026** (moteur testé ; UI à valider visuellement) : parseur — `<mark>` (Highlight) enfin lu, `<p></p>`/`<h*></h*>` vides matérialisés (les lignes vides survivaient pas au save/load), gardes `ReferenceEquals` sur `</ul>`/`</li>` (le contenu s'échappait des blockquotes/listes imbriquées), `DocImage.Title` ; mode strict — surcharge `Parse(html, options)` → `HtmlParseResult` (document identique au tolérant + diagnostics situés), contrôle structurel `<pre><code>` ; sérialiseur — strong/em, `<mark data-color>`, `<pre><code>`, `<tbody>`, constantes Link TipTap, `img title` ; câblage — `HDevRichEdit.LoadHtml(html, options)` = canal stockage (le setter `Html` reste Mail pour « répondre à un mail »). Tests : 13 échantillons `getHTML()` × 3 théories (acceptation stricte, point fixe, auto-conformité) + négatifs — **325 verts** (73 → 325 sur la passe).

~~**🟠 Nécessaires, taille moyenne**~~ ✅ **Faits le 14/07/2026** (validés) :
- **HDevNavBar** : rail vertical de modules (icône+label+badge compteur, mode compact, sélection)
- **HDevSearchControl** : sur HDevEditorBase — suggestions en popup (callback app), ↑↓/Enter, croix d'effacement, `SearchRequested`/`SuggestionChosen`
- **HDevTokenEdit** : chips avec ✕ + saisie inline, suggestions par callback, `TokenValidator` pour le texte libre, Enter/;/Tab valide, Backspace à vide retire le dernier
- **HDevToast** : `HDevToast.Show(fenêtre, titre, message, icône, durée, onClick)` — empilés bas-droite via OverlayLayer, auto-fermeture, clic actionnable
- **HDevMonthCalendar** : calendrier mensuel natif (◀ ▶, molette, pastille aujourd'hui, `BoldDates` = jours à événements, bornes, clavier) — **remplace le Calendar Fluent dans HDevDateEdit** (souveraineté)
- **Drag & drop applicatif** : HDevListView source (`EnableDragItems`, seuil 5 px, DataObject `procontrols-item`), HDevTreeView cible (`AllowDropItems`, surlignage du nœud, `ItemDropped`)
...
- ~~Raccourcis clavier réels~~ ✅ **Fait le 14/07/2026** : `HDevShortcutManager` par fenêtre (KeyGesture.Parse des `Shortcut`, dispatch en bulle = le contrôle focalisé garde la priorité), enregistrement auto HDevMenuBar (récursif, `RefreshShortcuts()`) + `HDevRibbonButton.Shortcut`, **Alt+lettre** ouvre les menus, **navigation clavier complète** dans les menus ouverts (↑↓ saute séparateurs/désactivés, Enter, ←→ entre menus et sous-menus, Échap), `IsEnabled` bloque réellement le clic. API publique pour les raccourcis applicatifs.
- ~~Dark mode HDevTheme~~ ✅ **Fait le 14/07/2026** (validé) : `HDevTheme` à palettes commutables — l'API historique (`HDevTheme.Background.Panel`…) est INTACTE, les tokens délèguent à la palette active (`HDevPalette.Light`/`Dark`, 56 couleurs) ; `HDevTheme.Variant` + `VariantChanged`. Propagation : HDevWindow suit (RequestedThemeVariant pour les Fluent internes + invalidation récursive), 6 contrôles à brushes construits s'abonnent (Card, TextBox, EditorBase, TokenEdit, HtmlView/RichEdit = re-layout), dropdowns re-brushés (Combo/Date/Search/Token/LookUp via `RefreshThemeBrushes()` virtuel du socle), HDevDataGrid via ThemeDictionaries du skin Yaru (+ extraction des couleurs en dur du thème de base, commit HDev.UI.DataGrid e04094c). Toggle 🌙 démo. Une 3e variante = une palette de plus.
- Restantes : ribbon avancé (backstage/galeries/QAT/onglets contextuels), rendu des colonnes Image/Bouton du HDevDataGrid + master-detail, impression (inexistante partout).
...
- **HDevDock (docking de panneaux, ajouté au backlog le 14/07/2026)** : panneaux ancrables/détachables/empilables façon IDE (VS Code/Rider) — cible : postes denses type dispatcher Synaxis. Décision d'architecture actée : **PAS de MDI** (étranger aux conventions Linux, structurellement incompatible Wayland — pas de positionnement global des fenêtres par l'app) ; le modèle Linux est SDI + onglets (HDevTabControl ✅) + splits (GridSplitter) + **docking** (le seul morceau manquant). Périmètre envisagé : zones d'ancrage (gauche/droite/bas + centre document), drag d'un panneau avec aperçu des cibles, empilement en onglets, redimensionnement, panneaux épinglés/auto-masqués, sérialisation du layout.
...
**Volume estimé** : ~6 contrôles nouveaux + 4 moyens + 2 monstres (HTML, RichEdit) dépassant à eux seuls les lots 1-4. **Ordre naturel si lancement** : HDevListView → NavBar/Search/TokenEdit → HDevScheduler → RichEdit → rendu HTML (décision HTML en dernier).

**⚑ 14/07/2026 : LES 4 CHANTIERS STRUCTURANTS SONT FAITS (v1 validées)** — rendu HTML (HDevHtmlView), HDevRichEdit, HDevListView, HDevScheduler. Restent les moyens (HDevNavBar, HDevSearchControl, HDevTokenEdit, HDevAlert/toasts, HDevMonthCalendar, drag & drop applicatif) et les dettes (raccourcis clavier réels, ribbon avancé, impression, dark mode) — puis le backend (IMAP/stockage/indexation) qui est l'autre moitié du produit.
