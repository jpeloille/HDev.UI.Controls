...
public class HDevNavBarItem
{
    private string? _badge;

    internal HDevNavBar? Owner { get; set; }
    ...
    public HDevNavBarItem() { }

    public HDevNavBarItem(string icon, string label)
    {
        Icon = icon;
        Label = label;
    }
...
/// </summary>
public class HDevNavBar : Control
{
    ...
    public ObservableCollection<HDevNavBarItem> Items { get; } = new();
    ...
    public HDevNavBarItem? SelectedItem
    ...
    static HDevNavBar()
    {
        FocusableProperty.OverrideDefaultValue<HDevNavBar>(true);
    }
    ...
    public HDevNavBarItem Add(string icon, string label)
    {
        var item = new HDevNavBarItem(icon, label);
        Items.Add(item);
        return item;
    }
        ...
        for (int i = 0; i < Items.Count; i++)
        {
            ...
            if (isSelected)
            {
                context.FillRectangle(new SolidColorBrush(
                    HDevTheme.WithOpacity(HDevTheme.Accent.Primary, 26)), rect);
            ...
            var iconColor = isSelected ? HDevTheme.Accent.Primary : HDevTheme.Text.Secondary;
            ...
            if (!string.IsNullOrEmpty(item.Badge))
            {
                ...
                context.DrawRectangle(new SolidColorBrush(HDevTheme.Accent.Error), null,
                    badgeRect, 7, 7);
