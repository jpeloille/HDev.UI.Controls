...
/// </summary>
public class HDevWindow : Window
{
    // ═══════════════════════════════════════════════════════════════
    // ÉLÉMENTS UI
    // ═══════════════════════════════════════════════════════════════
    ...
    private HDevWindowButton? _minimizeButton;
    private HDevWindowButton? _maximizeButton;
    private HDevWindowButton? _closeButton;
...
#pragma warning restore CS0169
    ...
    public static readonly StyledProperty<bool> UseNativeChromeProperty =
        AvaloniaProperty.Register<HDevWindow, bool>(nameof(UseNativeChrome), true);

    public static readonly StyledProperty<IBrush?> TitleBarBackgroundProperty =
        AvaloniaProperty.Register<HDevWindow, IBrush?>(nameof(TitleBarBackground));
    
    public static readonly StyledProperty<IBrush?> TitleBarForegroundProperty =
        AvaloniaProperty.Register<HDevWindow, IBrush?>(nameof(TitleBarForeground));
    
    public static readonly StyledProperty<double> TitleBarHeightProperty =
        AvaloniaProperty.Register<HDevWindow, double>(nameof(TitleBarHeight), 40);
    
    public static readonly StyledProperty<bool> ShowIconProperty =
        AvaloniaProperty.Register<HDevWindow, bool>(nameof(ShowIcon), true);
    ...
    public static readonly StyledProperty<object?> TitleBarContentProperty =
        AvaloniaProperty.Register<HDevWindow, object?>(nameof(TitleBarContent));

    /// <summary>
    /// true (défaut) : décorations de fenêtre du système (headerbar GNOME sous
    /// Ubuntu). false : chrome custom HDevWindow (barre de titre + boutons).
    ...
    public HDevWindow()
    {
        ...
        // de la grille, scrollbars...) suit la variante HDevTheme — et se met à
        ...
        TitleBarBackground = new SolidColorBrush(HDevTheme.Background.Toolbar);
        TitleBarForeground = new SolidColorBrush(HDevTheme.Text.Primary);
        Background = new SolidColorBrush(HDevTheme.Background.Window);
    ...
    private void ApplyThemeVariant()
        => RequestedThemeVariant = HDevTheme.IsDark
    ...
    private void OnThemeVariantChanged(object? sender, EventArgs e)
    {
        ...
        // Les contrôles custom-rendered relisent HDevTheme au prochain rendu :
    ...
    private void InitializeComponent()
    {
        ...
        _minimizeButton = new HDevWindowButton(HDevWindowButtonType.Minimize);
        _minimizeButton.Click += (s, e) => WindowState = WindowState.Minimized;
        _minimizeButton.Bind(HDevWindowButton.IsVisibleProperty, this.GetObservable(CanMinimizeProperty));
        _windowButtonsPanel.Children.Add(_minimizeButton);
        
        _maximizeButton = new HDevWindowButton(HDevWindowButtonType.Maximize);
        _maximizeButton.Click += (s, e) => ToggleMaximize();
        _maximizeButton.Bind(HDevWindowButton.IsVisibleProperty, this.GetObservable(CanMaximizeProperty));
        _windowButtonsPanel.Children.Add(_maximizeButton);
        
        _closeButton = new HDevWindowButton(HDevWindowButtonType.Close);
    ...
    private void OnTitleBarPointerPressed(object? sender, PointerPressedEventArgs e)
    {
        if (e.GetCurrentPoint(this).Properties.IsLeftButtonPressed)
        {
            ...
            if (e.Source is HDevWindowButton)
                return;
    ...
    private void ToggleMaximize()
    {
        if (WindowState == WindowState.Maximized)
        {
            ...
            _maximizeButton?.SetButtonType(HDevWindowButtonType.Maximize);
        }
        else
        {
            WindowState = WindowState.Maximized;
            _maximizeButton?.SetButtonType(HDevWindowButtonType.Restore);
        }
    }
    ...
    private void ApplyChromeMode()
    {
        ...
        else
        {
            ...
            // sont dessinés par HDevWindow (équivalent 12 de l'ancien NoChrome)
        ...
        if (change.Property == WindowStateProperty)
        {
            ...
            _maximizeButton?.SetButtonType(
                state == WindowState.Maximized 
                    ? HDevWindowButtonType.Restore 
                    : HDevWindowButtonType.Maximize);
...
public enum HDevWindowButtonType
{
    Minimize,
    Maximize,
    Restore,
...
public class HDevWindowButton : Control
{
    private HDevWindowButtonType _buttonType;
    private bool _isHovered;
    private bool _isPressed;
    
    public event EventHandler? Click;
    
    public HDevWindowButton(HDevWindowButtonType type)
    {
        _buttonType = type;
        Width = 26;
        Height = 26;
    ...
    public void SetButtonType(HDevWindowButtonType type)
    {
        _buttonType = type;
        InvalidateVisual();
    }
    ...
    public override void Render(DrawingContext context)
    {
        ...
        var bgColor = _isPressed ? HDevTheme.Background.ControlPressed
                    : _isHovered ? HDevTheme.Background.ControlHover
                    : HDevTheme.Background.Control;

        var center = bounds.Center;
        var radius = bounds.Width / 2;
        context.DrawEllipse(new SolidColorBrush(bgColor), null, center, radius, radius);

        var iconColor = HDevTheme.Text.Primary;
        var iconPen = new Pen(new SolidColorBrush(iconColor), 1.2);
        
        // Dessiner l'icône selon le type
        switch (_buttonType)
        {
            case HDevWindowButtonType.Minimize:
            ...
            case HDevWindowButtonType.Close:
                // Croix
                context.DrawLine(iconPen,
