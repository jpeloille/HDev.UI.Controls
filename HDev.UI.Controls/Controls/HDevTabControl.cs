...
/// <summary>
/// Page d'un HDevTabControl : en-tête + contenu
/// </summary>
public class HDevTabPage
{
    private string _header = "";
    private string? _icon;

    internal HDevTabControl? Owner { get; set; }
    ...
    public HDevTabPage(string header, Control? content = null, string? icon = null)
    {
        _header = header;
        Content = content;
        _icon = icon;
...
/// </summary>
public class HDevTabClosingEventArgs : System.ComponentModel.CancelEventArgs
{
    public HDevTabPage Page { get; }

    public HDevTabClosingEventArgs(HDevTabPage page)
    {
        Page = page;
    }
}
...
/// </summary>
public class HDevTabControl : Control
{
    ...
    private HDevTabPage? _hoveredTab;
    private HDevTabPage? _pressedTab;
    private HDevTabPage? _closeHoveredTab;
    private Control? _hostedContent;

    public ObservableCollection<HDevTabPage> Items { get; } = new();
    ...
    public event EventHandler<HDevTabClosingEventArgs>? TabClosing;

    /// <summary>Déclenché après la fermeture d'un onglet</summary>
    public event EventHandler<HDevTabPage>? TabClosed;
    ...
    public HDevTabPage? SelectedTab
        => _selectedIndex >= 0 && _selectedIndex < Items.Count ? Items[_selectedIndex] : null;

    static HDevTabControl()
    {
        FocusableProperty.OverrideDefaultValue<HDevTabControl>(true);
    }

    public HDevTabControl()
    {
        ClipToBounds = true;
    ...
    public HDevTabPage AddPage(string header, Control? content = null, string? icon = null)
    {
        var page = new HDevTabPage(header, content, icon);
        Items.Add(page);
        return page;
    }

    /// <summary>Ferme une page (après TabClosing, annulable)</summary>
    public void ClosePage(HDevTabPage page)
    {
        if (!Items.Contains(page)) return;

        var args = new HDevTabClosingEventArgs(page);
    ...
    private IEnumerable<(HDevTabPage Page, Rect Rect, Rect CloseRect)> GetTabsLayout()
    {
        double x = 8;
    ...
    private double MeasureTabWidth(HDevTabPage page)
    {
        var text = CreateText(GetTabText(page), HDevTheme.Text.Primary);
        return 12 + text.Width + 12 + (page.CanClose ? CloseSize + 4 : 0);
    }

    private static string GetTabText(HDevTabPage page)
        => string.IsNullOrEmpty(page.Icon) ? page.Header : $"{page.Icon} {page.Header}";

    private (HDevTabPage? Page, bool OnClose) HitTest(Point pos)
    {
