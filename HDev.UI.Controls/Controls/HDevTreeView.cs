...
/// <summary>
/// Nœud d'un HDevTreeView
/// </summary>
public class HDevTreeNode
{
    private string _text = "";
    private string? _icon;
    private bool _isExpanded;

    internal HDevTreeView? Owner { get; set; }
    public HDevTreeNode? Parent { get; internal set; }

    public ObservableCollection<HDevTreeNode> Children { get; } = new();
    ...
    public HDevTreeNode(string text, string? icon = null) : this()
    {
        _text = text;
        _icon = icon;
    }
    ...
    public HDevTreeNode Add(string text, string? icon = null)
    {
        var node = new HDevTreeNode(text, icon);
        Children.Add(node);
        return node;
    }

    internal void SetOwner(HDevTreeView? owner)
    {
        Owner = owner;
        foreach (var child in Children)
            child.SetOwner(owner);
...
/// </summary>
public class HDevTreeView : Control
{
    private const double RowHeight = 26;
    private const double IndentWidth = 18;

    private HDevTreeNode? _selectedNode;
    private HDevTreeNode? _hoveredNode;
    private HDevTreeNode? _dropTargetNode;
    private List<(HDevTreeNode Node, int Depth)> _visibleRows = new();

    public ObservableCollection<HDevTreeNode> Nodes { get; } = new();

    /// <summary>Déclenché quand la sélection change</summary>
    public event EventHandler? SelectedNodeChanged;

    /// <summary>Déclenché quand un nœud est développé</summary>
    public event EventHandler<HDevTreeNode>? NodeExpanded;

    /// <summary>Déclenché quand un nœud est replié</summary>
    public event EventHandler<HDevTreeNode>? NodeCollapsed;

    /// <summary>Déclenché au double-clic sur un nœud</summary>
    public event EventHandler<HDevTreeNode>? NodeDoubleClicked;

    /// <summary>Un item (HDevListView...) a été déposé sur un nœud</summary>
    public event EventHandler<(HDevTreeNode Node, object Item)>? ItemDropped;
    ...
    public HDevTreeNode? SelectedNode
    {
        get => _selectedNode;
        set
        {
    ...
    static HDevTreeView()
    {
        FocusableProperty.OverrideDefaultValue<HDevTreeView>(true);
    }
    ...
    public HDevTreeView()
    {
        ...
        {
            if (!_allowDropItems || !e.DataTransfer.Contains(HDevListView.DragDataFormat))
            {
        ...
        AddHandler(DragDrop.DropEvent, (s, e) =>
        {
            ...
            if (node != null && e.DataTransfer.TryGetValue(HDevListView.DragDataFormat) is { } item)
            {
    ...
    private static void SetExpandedRecursive(IEnumerable<HDevTreeNode> nodes, bool expanded)
    {
        foreach (var node in nodes)
        {
            if (node.HasChildren)
    ...
    internal void OnNodeExpandChanged(HDevTreeNode node)
    {
        RefreshLayout();
        if (node.IsExpanded)
    ...
    {
        _visibleRows = new List<(HDevTreeNode, int)>();
        Flatten(Nodes, 0);
        InvalidateMeasure();
        InvalidateVisual();
    }

    private void Flatten(IEnumerable<HDevTreeNode> nodes, int depth)
    {
        foreach (var node in nodes)
        {
    ...
    private (HDevTreeNode Node, int Depth, Rect Rect)? RowAt(Point pos)
    {
        ...
        for (int i = 0; i < _visibleRows.Count; i++)
        {
            ...
            if (ReferenceEquals(node, _dropTargetNode))
            {
                context.DrawRectangle(
                    new SolidColorBrush(HDevTheme.WithOpacity(HDevTheme.Accent.Primary, 24)),
                    new Pen(new SolidColorBrush(HDevTheme.Accent.Primary), 1.5),
                    rowRect.Deflate(new Thickness(2, 1)), 4, 4);
            }
            else if (isSelected)
            {
                context.FillRectangle(new SolidColorBrush(
                    HDevTheme.WithOpacity(HDevTheme.Accent.Primary, 30)), rowRect.Deflate(new Thickness(2, 1)), 4);
            }
            else if (isHovered)
            {
            ...
            var text = new FormattedText(label, CultureInfo.CurrentCulture,
                FlowDirection.LeftToRight, new Typeface(HDevTheme.Typography.FontFamily),
                HDevTheme.Typography.FontSizeBody,
                new SolidColorBrush(isSelected ? HDevTheme.Text.Primary : HDevTheme.Text.Primary));
