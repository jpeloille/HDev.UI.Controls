...
/// </summary>
public class HDevGantt : Control
{
    ...
    static HDevGantt()
    {
        FocusableProperty.OverrideDefaultValue<HDevGantt>(true);
    }
    ...
    public void BeginCellEdit(GanttTask task, int column)
    {
        ...
        {
            0 => new HDevTextBox { Text = task.Name },
            1 => new HDevDateEdit { Value = task.Start, MinWidth = 40 },
            2 => new HDevDateEdit { Value = task.End, MinWidth = 40 },
            3 => new HDevSpinEdit { MinValue = 0, MaxValue = 100, Decimals = 0, Value = (decimal)task.Progress, MinWidth = 40 },
            _ => null
        };
        ...
        Avalonia.Threading.Dispatcher.UIThread.Post(() =>
        {
            switch (_cellEditor)
            {
                case HDevTextBox tb: tb.FocusTextBox(); tb.SelectAll(); break;
                case HDevEditorBase eb: eb.FocusEditor(); eb.SelectAll(); break;
            }
    ...
    private void CloseCellEditor(bool commit)
    {
        ...
        if (commit && _project != null)
        {
            switch (column)
            {
                case 0 when editor is HDevTextBox tb:
                ...
                case 1 when editor is HDevDateEdit de && de.Value is { } newStart:
                    ApplyDatesEdit(task, newStart,
                        task.IsMilestone ? newStart : (task.End < newStart ? newStart : task.End));
                    break;

                case 2 when editor is HDevDateEdit de && de.Value is { } newEnd:
                    ApplyDatesEdit(task, task.Start > newEnd ? newEnd : task.Start, newEnd);
                    break;

                case 3 when editor is HDevSpinEdit spin:
    ...
    public override void Render(DrawingContext context)
    {
        ...
        {
            var empty = CreateText("Aucun projet", HDevTheme.Text.Secondary, 13);
    ...
    private void RenderDragOverlay(DrawingContext context)
    {
        ...
        using (context.PushClip(timelineRect))
        {
                ...
                _rowIndexByTask.TryGetValue(hovered, out var hoveredIndex))
            {
                ...
                if (!hovered.IsMilestone)
                {
                    ...
                    context.DrawEllipse(new SolidColorBrush(HDevTheme.Background.Panel),
                        new Pen(new SolidColorBrush(HDevTheme.Accent.Primary), 1.4),
                    ...
                    context.DrawGeometry(new SolidColorBrush(HDevTheme.Accent.Primary), null, grip);
                }
            }
                ...
                && _rowIndexByTask.TryGetValue(_dragTask, out var dragIndex))
            {
                ...
                var ghostPen = new Pen(new SolidColorBrush(HDevTheme.Accent.Primary), 1.5)
                {
                ...
                else
                {
                    context.DrawRectangle(
                        new SolidColorBrush(HDevTheme.WithOpacity(HDevTheme.Accent.Primary, 30)),
                        ghostPen,
                ...
                var text = CreateText(label, HDevTheme.Text.Primary, 12);
                var labelPos = new Point(gx1, y - BarHeight / 2 - text.Height - 4);
                context.FillRectangle(new SolidColorBrush(HDevTheme.Background.Panel),
                ...
                && _rowIndexByTask.TryGetValue(_dragTask, out var progressIndex))
            {
                ...
                context.DrawLine(new Pen(new SolidColorBrush(HDevTheme.Accent.Primary), 2),
                    new Point(px, y - BarHeight / 2 - 3), new Point(px, y + BarHeight / 2 + 3));

                var text = CreateText($"{_ghostProgress:F0} %", HDevTheme.Text.Primary, 12);
            ...
            if (_dragKind == DragKind.Link)
            {
                ...
                if (_rowIndexByTask.TryGetValue(_dragTask, out var linkIndex))
                {
                    ...
                    var color = _linkTarget == null
                        ...
                        : _linkTargetValid ? HDevTheme.Accent.Success : HDevTheme.Accent.Error;
    ...
    private void RenderHeader(DrawingContext context)
    {
        ...
        {
            var text = CreateText(caption, HDevTheme.Text.Secondary, 12);
        ...
        {
            var tickPen = new Pen(new SolidColorBrush(HDevTheme.Border.Subtle), 1);
            ...
            foreach (var tick in Axis.MajorTicks(TimelineWidth))
            {
                ...
                var text = CreateText(tick.Label, HDevTheme.Text.Primary, 12);
            ...
            foreach (var tick in Axis.MinorTicks(TimelineWidth))
            {
                ...
                var text = CreateText(tick.Label, HDevTheme.Text.Secondary, 10);
    ...
    private void RenderTimelineBackground(DrawingContext context)
    {
        ...
        using (context.PushClip(timelineRect))
        {
            ...
            {
                var shade = new SolidColorBrush(HDevTheme.WithOpacity(HDevTheme.Text.Secondary, 14));
            ...
            var gridPen = new Pen(new SolidColorBrush(HDevTheme.WithOpacity(HDevTheme.Border.Subtle, 120)), 1);
            foreach (var tick in Axis.MinorTicks(TimelineWidth))
            {
                var tx = TimelineX + tick.X;
    ...
    private void RenderRowsBackground(DrawingContext context, int first, int last)
    {
        ...
        var rowPen = new Pen(new SolidColorBrush(HDevTheme.WithOpacity(HDevTheme.Border.Subtle, 90)), 1);
        ...
        for (int i = first; i <= last; i++)
        {
            ...
            if (ReferenceEquals(task, _selectedTask))
            {
                context.FillRectangle(new SolidColorBrush(
                    HDevTheme.WithOpacity(HDevTheme.Accent.Primary, 26)), rowRect);
            }
            else if (ReferenceEquals(task, _hoveredTask))
            {
                context.FillRectangle(new SolidColorBrush(HDevTheme.Background.ControlHover), rowRect);
            }
        ...
        using (context.PushClip(new Rect(0, HeaderHeight, _tableWidth, RowsViewport.Height)))
        {
            for (int i = first; i <= last; i++)
            {
                ...
                if (isSummary)
                {
                    ...
                    var pen = new Pen(new SolidColorBrush(HDevTheme.Text.Secondary), 1.4);
                ...
                var nameText = new FormattedText(task.Name, CultureInfo.CurrentCulture,
                    ...
                    13, new SolidColorBrush(HDevTheme.Text.Primary));
                ...
                {
                    var cellText = CreateText(cells[c], HDevTheme.Text.Secondary, 12);
            ...
            for (int i = first; i <= last; i++)
            {
                ...
                if (_showBaseline && task.HasBaseline)
                {
                    ...
                    var baselineBrush = new SolidColorBrush(
                        HDevTheme.WithOpacity(HDevTheme.Text.Secondary, 110));
                ...
                if (task.IsMilestone)
                {
                    ...
                    context.DrawGeometry(new SolidColorBrush(
                        isCritical ? HDevTheme.Accent.Error : HDevTheme.Accent.Secondary), null, geometry);
                }
                else if (task.IsSummary)
                {
                    // Crochet récapitulatif (barre fine + pattes)
                    var brush = new SolidColorBrush(
                        isCritical ? HDevTheme.Accent.Error : HDevTheme.Text.Primary);
                    var barY = centerY - 5;
                ...
                else
                {
                    ...
                    var barColor = isCritical ? HDevTheme.Accent.Error : HDevTheme.Accent.Primary;
                    context.DrawRectangle(
                        new SolidColorBrush(HDevTheme.WithOpacity(barColor, 70)),
    ...
    {
        var timelineRect = new Rect(TimelineX, HeaderHeight, TimelineWidth, RowsViewport.Height);
        var brush = new SolidColorBrush(HDevTheme.WithOpacity(HDevTheme.Text.Secondary, 170));
    ...
    private void RenderNowLine(DrawingContext context)
    {
        ...
        context.DrawLine(
            new Pen(new SolidColorBrush(HDevTheme.Accent.Error), 1.5),
    ...
    private static FormattedText CreateText(string text, Color color, double size)
        => new(text, CultureInfo.CurrentCulture, FlowDirection.LeftToRight,
            new Typeface(HDevTheme.Typography.FontFamily), size, new SolidColorBrush(color));
}
