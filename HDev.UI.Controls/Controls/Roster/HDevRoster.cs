...
/// </summary>
/// <remarks>
/// <b>Ce que ce contrôle n'est pas.</b> <c>HDevGantt</c> est un planificateur de projet — WBS,
/// dépendances, chemin critique — et non un planning de ressources. <c>HDevScheduler</c> est un
...
/// <c>HDevTheme</c>, donc la bascule clair/sombre suit. Les pinceaux sont alloués <b>à chaque
/// frame</b> — comme HDevGantt et HDevScheduler, et contrairement à la frise d'origine, dont les
/// caches statiques figeaient les couleurs au changement de thème.</para>
/// </remarks>
public class HDevRoster : Control
{
    ...
    static HDevRoster()
    {
        FocusableProperty.OverrideDefaultValue<HDevRoster>(true);
    }
    ...
    public override void Render(DrawingContext context)
    {
        ...
        {
            var empty = CreateText("Aucune voie à afficher", HDevTheme.Text.Secondary, 13);
    ...
    private void RenderWeekendShading(DrawingContext context, int first, int last)
    {
        ...
        var brush = new SolidColorBrush(HDevTheme.Background.PanelAlt);
    ...
    private void RenderHoveredLane(DrawingContext context, int first, int last)
    {
        ...
        context.FillRectangle(
            new SolidColorBrush(HDevTheme.Background.Selection, 0.28),
    ...
    {
        var pen = new Pen(new SolidColorBrush(HDevTheme.Border.Subtle), 1);
    ...
    {
        var neutral = HDevTheme.Background.ControlHover;
        var borderPen = new Pen(new SolidColorBrush(HDevTheme.Border.Default), 1);
        var markerPen = new Pen(new SolidColorBrush(HDevTheme.Text.Primary), 2);
    ...
    private void RenderBandLabels(DrawingContext context, RosterBand band, Rect rect)
    {
        ...
        {
            var label = CreateText(band.Label, HDevTheme.Text.Primary, 10);
        ...
        var start = band.StartLabel is { Length: > 0 }
            ? CreateText(band.StartLabel, HDevTheme.Text.Secondary, 9)
            : null;

        var end = band.EndLabel is { Length: > 0 }
            ? CreateText(band.EndLabel, HDevTheme.Text.Secondary, 9)
            : null;
    ...
    {
        var neutral = HDevTheme.Accent.Success;
    ...
    private void RenderBlocks(DrawingContext context, int first, int last)
    {
        ...
        var neutral = HDevTheme.Background.Control;
        var neutralPen = new Pen(new SolidColorBrush(HDevTheme.Border.Strong), 1);
        var selectionPen = new Pen(new SolidColorBrush(HDevTheme.Border.Focused), 2);
        var hoverPen = new Pen(new SolidColorBrush(HDevTheme.Accent.Primary), 1.5);
    ...
    private static void RenderBlockLabel(DrawingContext context, RosterBlock block, Rect rect)
    {
        ...
        var label = CreateText(
            block.Label,
            block.Color is null ? HDevTheme.Text.Primary : HDevTheme.Text.OnAccent,
            9);
    ...
    private void RenderNowLine(DrawingContext context, int first, int last)
    {
        ...
        var now = NowInstant;
        var pen = new Pen(new SolidColorBrush(HDevTheme.Accent.Error), 2);
    ...
    {
        context.FillRectangle(
            new SolidColorBrush(HDevTheme.Background.Panel),
            new Rect(0, HeaderHeight, GutterWidth, RowsViewport.Height));

        var separator = new Pen(new SolidColorBrush(HDevTheme.Border.Default), 1);
        ...
        for (var i = first; i <= last; i++)
        {
            ...
            var label = CreateText(
                lane.Label,
                lane.HasWarning ? HDevTheme.Text.Warning : HDevTheme.Text.Primary,
                11);
            ...
            {
                var sub = CreateText(lane.SubLabel, HDevTheme.Text.Secondary, 9);
    ...
    private void RenderHeader(DrawingContext context)
    {
        ...
        context.FillRectangle(new SolidColorBrush(HDevTheme.Background.Toolbar), header);

        var gridPen = new Pen(new SolidColorBrush(HDevTheme.Border.Subtle), 1);
        var borderPen = new Pen(new SolidColorBrush(HDevTheme.Border.Default), 1);
        ...
        using (context.PushClip(new Rect(TimelineX, 0, TimelineWidth, HeaderHeight)))
        {
            ...
            if (twoLevels)
            {
                foreach (var tick in Axis.MajorTicks(TimelineWidth, LaneSpan))
                {
                    ...
                    context.DrawText(
                        CreateText(tick.Label, HDevTheme.Text.Primary, 11),
            ...
            foreach (var tick in Axis.MinorTicks(TimelineWidth, LaneSpan))
            {
                ...
                var label = CreateText(tick.Label, HDevTheme.Text.Secondary, 12);
    ...
    private void RenderDragGhost(DrawingContext context)
    {
        ...
        var accent = HDevTheme.Accent.Primary;
        ...
        var label = CreateText(_dragStart.ToString("dd/MM HH:mm"), HDevTheme.Text.Primary, 10);
