...
/// </summary>
public class HDevRibbon : Control
{
    ...
    private HDevRibbonButton? _hoveredButton;
    private HDevRibbonButton? _pressedButton;
    private HDevRibbonGroup? _hoveredLauncher;
    private HDevRibbonGroup? _pressedLauncher;
    ...
    public void RefreshShortcuts()
    {
        ...
        var manager = HDevShortcutManager.GetFor(root);
        foreach (var tab in Tabs)
        {
            foreach (var group in tab.Groups)
            {
                ...
                {
                    if (item is not HDevRibbonButton button ||
                        HDevShortcutManager.TryParse(button.Shortcut) is not { } gesture)
                        continue;
    ...
    public static readonly StyledProperty<int> SelectedTabIndexProperty =
        AvaloniaProperty.Register<HDevRibbon, int>(nameof(SelectedTabIndex), 0);
    
    public static readonly StyledProperty<bool> IsCollapsedProperty =
        AvaloniaProperty.Register<HDevRibbon, bool>(nameof(IsCollapsed), false);
    ...
    public ObservableCollection<HDevRibbonTab> Tabs { get; } = new();
    
    public HDevRibbonTab? SelectedTab => 
    ...
    static HDevRibbon()
    {
        AffectsRender<HDevRibbon>(SelectedTabIndexProperty, IsCollapsedProperty);
        AffectsMeasure<HDevRibbon>(IsCollapsedProperty);
    }
    ...
    private HDevRibbonButton? GetButtonAtPosition(Point pos)
    {
        if (IsCollapsed || pos.Y <= TabRowHeight) return null;

        foreach (var (item, rect) in GetContentLayout())
        {
            if (item is HDevRibbonButton btn && rect.Contains(pos))
                return btn;
        }
        return null;
    }
    ...
    /// </summary>
    private IEnumerable<(HDevRibbonGroup Group, Rect Rect)> GetLauncherLayout()
    {
    ...
    private HDevRibbonGroup? GetLauncherAtPosition(Point pos)
    {
        if (IsCollapsed || pos.Y <= TabRowHeight) return null;
    ...
    /// </summary>
    internal IEnumerable<(HDevRibbonItem Item, Rect Rect)> GetContentLayout()
    {
        if (SelectedTab == null) yield break;
    ...
    private IEnumerable<(HDevRibbonItem Item, Rect Rect)> GetGroupItemsLayout(
        HDevRibbonGroup group, double x, double y)
    {
        ...
        foreach (var item in group.Items)
        {
            if (item is HDevRibbonButton btn)
            {
                if (btn.IsLarge)
                {
                    if (smallRow > 0)
            ...
            else if (item is HDevRibbonSeparator sep)
            {
                if (smallRow > 0)
                {
                    itemX = smallColumnX + maxSmallWidth + 4;
    ...
    private double MeasureTabWidth(HDevRibbonTab tab)
    {
        var text = CreateFormattedText(tab.Header, HDevTheme.Text.Primary, 12);
    ...
    public override void Render(DrawingContext context)
    {
        ...
        // Bordure en bas
        var borderPen = new Pen(new SolidColorBrush(HDevTheme.Border.Subtle), 1);
    ...
    private void RenderTabs(DrawingContext context, Rect bounds)
    {
        ...
        for (int i = 0; i < Tabs.Count; i++)
        {
            ...
            if (isSelected)
            {
                context.DrawRectangle(
                    new SolidColorBrush(HDevTheme.Background.ControlHover), null,
                    pillRect, 6, 6);
            }
            else if (isHovered)
            {
                context.DrawRectangle(
                    new SolidColorBrush(HDevTheme.Background.Control), null,
                    pillRect, 6, 6);
            }
        ...
        if (!IsCollapsed)
        {
            var sepPen = new Pen(new SolidColorBrush(HDevTheme.Border.Subtle), 1);
    ...
    private void RenderCollapseButton(DrawingContext context, Rect bounds)
    {
        ...
        var btnY = 6;
        var pen = new Pen(new SolidColorBrush(HDevTheme.Text.Tertiary), 1.5);
    ...
    private void RenderContent(DrawingContext context, Rect bounds)
    {
        ...
        // Fond du contenu
        context.FillRectangle(new SolidColorBrush(HDevTheme.Background.Panel), contentRect);
    ...
    private double MeasureGroupWidth(HDevRibbonGroup group)
    {
        ...
        foreach (var item in group.Items)
        {
            ...
            else if (item is HDevRibbonSeparator)
            {
                width += 12;
            }
        }
        ...
        var labelText = CreateFormattedText(group.Header, HDevTheme.Text.Tertiary, 11);
        width = Math.Max(width, labelText.Width + 20);
        
        return width;
    }
    
    private double MeasureSmallButtonWidth(HDevRibbonButton btn)
    {
        var text = CreateFormattedText(btn.Label.Replace("\n", " "), HDevTheme.Text.Primary, 11);
        return 22 + text.Width + 8; // icône + texte + padding
    }
    
    private void RenderGroup(DrawingContext context, HDevRibbonGroup group, 
        double x, double y, double width)
    {
        // Séparateur à droite
        var sepPen = new Pen(new SolidColorBrush(HDevTheme.Border.Default), 1);
        context.DrawLine(sepPen,
        ...
        var labelLinePen = new Pen(new SolidColorBrush(HDevTheme.Border.Subtle), 1);
    ...
    private void RenderDialogLauncher(DrawingContext context, HDevRibbonGroup group, Rect rect)
    {
        ...
        if (isHovered)
        {
            context.FillRectangle(new SolidColorBrush(isPressed
                ? HDevTheme.Background.ControlPressed
                : HDevTheme.Background.ControlHover), rect, 3);
        }

        // Glyphe : coin haut-gauche + flèche diagonale vers le bas-droit
        var pen = new Pen(new SolidColorBrush(HDevTheme.Text.Secondary), 1.2);
        var cx = rect.X + 4;
        var cy = rect.Y + 4;
        var ax = rect.Right - 4;
    ...
    private void RenderButtonBackground(DrawingContext context, HDevRibbonButton btn, Rect btnRect, float radius)
    {
        ...
        if (isPressed || btn.IsChecked)
        {
            context.FillRectangle(
                new SolidColorBrush(HDevTheme.Background.ControlPressed),
                btnRect, radius);

            if (btn.IsChecked)
            {
                var checkedPen = new Pen(new SolidColorBrush(HDevTheme.Accent.Primary), 1);
        ...
        {
            context.FillRectangle(
                new SolidColorBrush(HDevTheme.Background.ControlHover),
                btnRect, radius);
        }
    }

    private void RenderLargeButton(DrawingContext context, HDevRibbonButton btn, Rect btnRect)
    {
    ...
    private void RenderSmallButton(DrawingContext context, HDevRibbonButton btn, Rect btnRect)
    {
