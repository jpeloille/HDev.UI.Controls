...
/// </summary>
public class HDevListItemContent
{
...
public class HDevListHoverAction
{
    public string Glyph { get; }
    public string Tooltip { get; }
    public Action<object> Execute { get; }

    public HDevListHoverAction(string glyph, string tooltip, Action<object> execute)
    {
        Glyph = glyph;
        Tooltip = tooltip;
        Execute = execute;
...
/// </summary>
public class HDevListView : Control
{
    ...
    public Func<object, HDevListItemContent>? ItemAdapter { get; set; }
    ...
    public List<HDevListHoverAction> HoverActions { get; } = new();
    ...
    /// <summary>Les items peuvent être glissés (drag &amp; drop vers un HDevTreeView...)</summary>
    ...
    static HDevListView()
    {
        FocusableProperty.OverrideDefaultValue<HDevListView>(true);
    }
    ...
    public override void Render(DrawingContext context)
    {
        ...
        if (_rows.Count == 0)
        {
            var empty = Text(EmptyText, HDevTheme.Text.Secondary, 13);
    ...
    private void RenderHeader(DrawingContext context, Row row, double y, double width)
    {
        ...
        var label = Text($"{row.GroupKey}  ({row.GroupCount})", HDevTheme.Text.Secondary, 12, FontWeight.SemiBold);
    ...
    {
        var content = ItemAdapter?.Invoke(item) ?? new HDevListItemContent { Title = item.ToString() ?? "" };
        var isSelected = _selected.Contains(item);
        var isHovered = rowIndex == _hoveredRow;
        var rowRect = new Rect(0, y, width, _rowHeight);

        if (isSelected)
            context.FillRectangle(new SolidColorBrush(HDevTheme.WithOpacity(HDevTheme.Accent.Primary, 28)), rowRect);
        else if (isHovered)
        ...
        if (content.Emphasized)
            context.FillRectangle(new SolidColorBrush(HDevTheme.Accent.Primary),
        ...
        if (!string.IsNullOrEmpty(content.Icon))
        {
            ...
            context.DrawEllipse(new SolidColorBrush(HDevTheme.Background.Toolbar),
                new Pen(new SolidColorBrush(HDevTheme.Border.Subtle), 1),
                circle.Center, 16, 16);
            var icon = Text(content.Icon, HDevTheme.Text.Primary, 13, FontWeight.SemiBold);
        ...
        {
            var trailing = Text(content.Trailing, HDevTheme.Text.Secondary, 11);
        ...
        {
            var subtitle = Text(content.Subtitle, HDevTheme.Text.Secondary, 12);
        ...
        if (!showActions && !string.IsNullOrEmpty(content.Badge))
        {
            ...
            context.DrawRectangle(new SolidColorBrush(HDevTheme.Accent.Primary), null, badgeRect, 7.5, 7.5);
        ...
        if (showActions)
        {
            ...
            for (int a = 0; a < HoverActions.Count; a++)
            {
                var rect = rects[a];
                if (a == _hoveredAction)
                    context.DrawEllipse(new SolidColorBrush(HDevTheme.Background.ControlPressed),
                        null, rect.Center, ActionSize / 2, ActionSize / 2);

                var glyph = Text(HoverActions[a].Glyph, HDevTheme.Text.Primary, 13);
        ...
        // Séparateur
        context.DrawLine(new Pen(new SolidColorBrush(HDevTheme.WithOpacity(HDevTheme.Border.Subtle, 110)), 1),
