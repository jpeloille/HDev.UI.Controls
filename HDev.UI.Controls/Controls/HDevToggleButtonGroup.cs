...
/// <summary>Segment d'un HDevToggleButtonGroup</summary>
public class HDevToggleButtonItem
{
    private string _text = "";
    private bool _isSelected;

    internal HDevToggleButtonGroup? Owner { get; set; }
    ...
    public HDevToggleButtonItem() { }

    public HDevToggleButtonItem(string text, string? icon = null)
    {
        _text = text;
        Icon = icon;
    }
...
/// </summary>
public class HDevToggleButtonGroup : Control
{
    private const double GroupHeight = 30;

    private HDevToggleButtonItem? _hoveredItem;
    private HDevToggleButtonItem? _pressedItem;
    private int _focusIndex = -1;

    public ObservableCollection<HDevToggleButtonItem> Items { get; } = new();
    ...
    public IEnumerable<HDevToggleButtonItem> SelectedItems => Items.Where(i => i.IsSelected);

    static HDevToggleButtonGroup()
    {
        FocusableProperty.OverrideDefaultValue<HDevToggleButtonGroup>(true);
    }

    public HDevToggleButtonGroup()
    {
    ...
    public HDevToggleButtonItem Add(string text, string? icon = null, bool isSelected = false)
    {
        var item = new HDevToggleButtonItem(text, icon);
        Items.Add(item);
    ...
    internal void SetSelected(HDevToggleButtonItem item, bool selected)
    {
    ...
    private string DisplayText(HDevToggleButtonItem item)
        => string.IsNullOrEmpty(item.Icon) ? item.Text
            : string.IsNullOrEmpty(item.Text) ? item.Icon! : $"{item.Icon} {item.Text}";

    private double SegmentWidth(HDevToggleButtonItem item)
        => 14 + CreateText(DisplayText(item), HDevTheme.Text.Primary).Width + 14;

    private IEnumerable<(HDevToggleButtonItem Item, Rect Rect)> SegmentsLayout()
    {
        double x = 0;
        foreach (var item in Items)
        {
    ...
    private HDevToggleButtonItem? HitTestSegment(Point pos)
    {
    ...
    private void Toggle(HDevToggleButtonItem item)
    {
        if (SelectionMode == ToggleGroupSelectionMode.Single)
    ...
    public override void Render(DrawingContext context)
    {
        ...
        // Enveloppe pilule
        context.DrawRectangle(new SolidColorBrush(HDevTheme.Background.Toolbar),
        ...
        using (context.PushGeometryClip(new RectangleGeometry(bounds, radius, radius)))
        {
            ...
            foreach (var (item, rect) in SegmentsLayout())
            {
                ...
                if (isSelected)
                {
                    context.FillRectangle(new SolidColorBrush(HDevTheme.Accent.Primary), rect);
                }
                else if (isPressed)
                {
                    context.FillRectangle(new SolidColorBrush(HDevTheme.Background.ControlPressed), rect);
                }
                else if (isHovered)
                {
                    context.FillRectangle(new SolidColorBrush(HDevTheme.Background.ControlHover), rect);
                }
                ...
                var textColor = !item.IsEnabled ? HDevTheme.Text.Disabled
                    : isSelected ? HDevTheme.Text.OnAccent
                ...
                if (IsFocused && index == _focusIndex)
                {
                    context.DrawRectangle(null,
                        new Pen(new SolidColorBrush(HDevTheme.WithOpacity(HDevTheme.Border.FocusOuter, 130)), 1.5),
    ...
    private static FormattedText CreateText(string text, Color color, FontWeight weight = FontWeight.Regular)
        => new(text, CultureInfo.CurrentCulture, FlowDirection.LeftToRight,
            ...
            HDevTheme.Typography.FontSizeBody - 1, new SolidColorBrush(color));
}
