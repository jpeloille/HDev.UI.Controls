...
/// <summary>
/// Popup contenant une liste de HDevMenuItem
/// Utilisé par HDevMenu et HDevContextMenu
/// </summary>
public class HDevMenuPopup : Popup
{
    ...
    /// </summary>
    internal HDevMenuPopup? ParentPopup { get; set; }
    ...
    public void SetItems(IEnumerable<HDevMenuItem> sourceItems)
    {
    ...
    // ═══════════════════════════════════════════════════════════════
    // NAVIGATION CLAVIER (pilotée par HDevMenuBar)
    // ═══════════════════════════════════════════════════════════════

    private int _highlightIndex = -1;

    internal IReadOnlyList<HDevMenuItem> MenuItems
        => _itemsPanel.Children.OfType<HDevMenuItem>().ToList();

    internal HDevMenuItem? HighlightedItem
    {
        get
        {
            var items = MenuItems;
    ...
    internal HDevMenuPopup? ActivateHighlighted()
    {
    ...
    internal HDevMenuPopup? OpenHighlightedSubmenu()
    {
        var item = HighlightedItem;
...
/// </summary>
public class HDevMenuBar : Panel
{
    private HDevMenuBarItem? _openItem;
    ...
    private HDevMenuPopup? _activePopup; // popup le plus profond (sous-menus)

    public ObservableCollection<HDevMenuBarItem> Items { get; } = new();

    public HDevMenuBar()
    {
        Height = 34;
        ClipToBounds = false;
        Background = new SolidColorBrush(HDevTheme.Background.Toolbar);
    ...
    public void RefreshShortcuts()
    {
        ...
        var manager = HDevShortcutManager.GetFor(_root);

        void Walk(IEnumerable<HDevMenuItem> items)
        {
            foreach (var item in items)
            {
                if (!item.IsSeparator &&
                    HDevShortcutManager.TryParse(item.Shortcut) is { } gesture)
                {
                    var captured = item;
    ...
    private void OpenMenuWithKeyboard(HDevMenuBarItem item)
    {
        OpenMenu(item);
        item.OpenMenu();
    ...
    internal void OpenMenu(HDevMenuBarItem item)
    {
    ...
    internal void OnItemHovered(HDevMenuBarItem item)
    {
...
/// </summary>
public class HDevMenuBarItem : Control
{
    ...
    public static readonly StyledProperty<string> HeaderProperty =
        AvaloniaProperty.Register<HDevMenuBarItem, string>(nameof(Header), "");
    ...
    static HDevMenuBarItem()
    {
        AffectsRender<HDevMenuBarItem>(HeaderProperty);
        AffectsMeasure<HDevMenuBarItem>(HeaderProperty);
    }
    
    public HDevMenuBarItem()
    {
        ClipToBounds = false;
    }
    
    public HDevMenuBarItem(string header) : this()
    {
        Header = header;
    }
    ...
    protected override void OnPointerEntered(PointerEventArgs e)
    {
        ...
        if (Parent is HDevMenuBar menuBar)
    ...
    internal void OpenMenu()
    {
        ...
        _popup.Closed += (s, e) =>
        {
            ...
            if (s is HDevMenuPopup p && p.WasClosedByGuard)
    ...
    internal HDevMenuPopup? CurrentPopup => _popup;
