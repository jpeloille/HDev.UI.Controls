...
public class ProMaskEngineTests
{
        ...
        string initial = "", int caret = -1)
    {
        ...
        {
            if (!ProMaskEngine.TryInsert(mask, text, pos, ch.ToString(), out text!, out pos))
                return (text, false);
        }
        return (text, true);
    }
    ...
    public void IsComplete_Date(string text, bool expected)
        => Assert.Equal(expected, ProMaskEngine.IsComplete("00/00/0000", text));

    [Fact]
    public void IsComplete_OptionalDigit_IsNotRequired()
        // 9 = chiffre optionnel : un texte sans lui reste complet
        => Assert.True(ProMaskEngine.IsComplete("009", "12"));
    ...
    public void Matches_CharacterClasses(char maskChar, char input, bool expected)
        => Assert.Equal(expected, ProMaskEngine.Matches(maskChar, input));
    ...
    public void IsPlaceholder(char maskChar, bool expected)
        => Assert.Equal(expected, ProMaskEngine.IsPlaceholder(maskChar));
    ...
    {
        var ok = ProMaskEngine.TryInsert("00/00/0000", "", 0, "10072026", out var text, out var caret);
        Assert.True(ok);
        Assert.Equal("10/07/2026", text);
    ...
    public void MultiCharInsert_WithInvalidChar_IsRejectedAtomically()
        => Assert.False(ProMaskEngine.TryInsert("00/00/0000", "", 0, "10a7", out _, out _));
}
