...
using HDev.UI.Controls.Demo.Models;
using System.Collections.Generic;
using System.Collections.ObjectModel;
using System.Linq;

namespace HDev.UI.Controls.Demo;
...
public partial class MainWindow : HDevWindow
{
    ...
    {
        var tabs = this.FindControl<HDevTabControl>("DemoTabs");
        if (tabs == null) return;

        // Page 1 : arborescence services -> employés (données de la démo)
        var tree = new HDevTreeView();
        ...
        {
            var statusBar = this.FindControl<HDevStatusBar>("MainStatusBar");
        ...
        var addButton = new HDevButton { Text = "Ajouter une page fermable", Variant = ButtonVariant.Secondary };
        aboutPanel.Children.Add(addButton);
        tabs.AddPage("Infos", aboutPanel, "ℹ️");

        // Page 3 : HDevGantt (lot 4b)
        tabs.AddPage("Projet", BuildGanttDemo(), "📊");

        // Page 4 : HDevHtmlView (monstre n° 1, tête de lecture)
        tabs.AddPage("Mail", BuildHtmlViewDemo(), "📧");

        // Page 5 : HDevRichEdit (monstre n° 2, tête d'édition)
        tabs.AddPage("Composer", BuildRichEditDemo(), "✍️");

        // Page 6 : mini-Outlook (HDevListView + HDevHtmlView)
        tabs.AddPage("Boîte", BuildInboxDemo(), "📥");

        // Page 7 : HDevScheduler (4e chantier structurant Outlook)
        tabs.AddPage("Agenda", BuildSchedulerDemo(), "📅");

        // Page 8 : HDevRoster (frise roster — voies x temps)
        tabs.AddPage("Roster", BuildRosterDemo(), "🗓");

        // Page 9 : HDevMindMap (brainstorming)
        tabs.AddPage("Carte", BuildMindMapDemo(), "🧠");

        // Page 9 : HDevDock (docking, phase 1) — poste dispatcher
    ...
    private Avalonia.Controls.Control BuildWidgetsDemo()
    {
        ...
        // ── HDevAccordion (volet gauche) : équipage + filtres + à propos ──
        var accordion = new HDevAccordion { Width = 280 };
        ...
        })
        {
            ...
            row.Children.Add(new HDevAvatar { FullName = name, Status = status });
        ...
        {
            var chip = new HDevChip { Text = label, Icon = icon, IsCheckable = true, Margin = new Avalonia.Thickness(2) };
        ...
        var alertChip = new HDevChip { Text = "NOTAM", Icon = "⚠️", Badge = "3", BadgeColor = HDev.UI.Controls.Theme.HDevTheme.Accent.Error };
        alertChip.Click += (s, e) => SetStatus("3 NOTAM actifs sur NWWW");
        alertPanel.Children.Add(alertChip);
        var okChip = new HDevChip { Text = "Messages", Icon = "✉️", Badge = "12" };
        alertPanel.Children.Add(okChip);
        var alertSection = accordion.AddSection("Alertes", alertPanel, "🔔");
        alertSection.HeaderBackground = HDev.UI.Controls.Theme.HDevTheme.WithOpacity(
            HDev.UI.Controls.Theme.HDevTheme.Accent.Error, 36);
        ...
        avatarRow.Children.Add(new HDevAvatar { FullName = "Julien Peloille", Size = AvatarSize.Small });
        avatarRow.Children.Add(new HDevAvatar { FullName = "Awa Wamytan", Size = AvatarSize.Medium, Status = AvatarStatus.Online });
        avatarRow.Children.Add(new HDevAvatar { FullName = "Teiki Brothers", Size = AvatarSize.Large, Status = AvatarStatus.Busy });
        avatarRow.Children.Add(new HDevAvatar { Initials = "NC", Size = AvatarSize.Large, Background = Avalonia.Media.Color.Parse("#00847E") });
        right.Children.Add(new HDevCard
        {
            Title = "HDevAvatar — tailles, statuts, couleur stable par nom",
            Content = avatarRow
        });
        ...
        {
            var chip = new HDevChip { Text = dest, Icon = "✉️", CanClose = true, Margin = new Avalonia.Thickness(2) };
            chip.Closed += (s, e) =>
            {
        ...
        right.Children.Add(new HDevCard
        {
            Title = "HDevChip — fermables (la croix retire le chip)",
            Content = chipsPanel
        });

        // Groupes segmentés
        var togglePanel = new Avalonia.Controls.StackPanel { Spacing = 10 };
        var viewGroup = new HDevToggleButtonGroup();
        viewGroup.Add("Jour");
        ...
        var styleGroup = new HDevToggleButtonGroup { SelectionMode = ToggleGroupSelectionMode.Multiple };
        ...
        right.Children.Add(new HDevCard
        {
            Title = "HDevToggleButtonGroup — Single (vue agenda) et Multiple (styles)",
            Content = togglePanel
        });
        ...
        {
            Width = 280,
            BorderBrush = new Avalonia.Media.SolidColorBrush(HDev.UI.Controls.Theme.HDevTheme.Border.Subtle),
    ...
    private Avalonia.Controls.Control BuildDockDemo()
    {
        ...
        dock.AddPanel(new HDevDockPanel("vol-ty201", "TY201",
            Pad("Plan de vol TY201 — Nouméa → Lifou\n\nZone document centrale : les volets ajoutés au centre s'empilent en onglets."),
            "🛫"), DockRegion.Center);
        dock.AddPanel(new HDevDockPanel("vol-ty340", "TY340",
            Pad("Plan de vol TY340 — Nouméa → Wallis"), "🛫"), DockRegion.Center);

        // Volet gauche : la flotte (HDevTreeView, dogfooding).
        var fleet = new HDevTreeView();
        ...
        atr.Add("F-OIQA", "🟢");
        atr.Add("F-OIQB", "⚪");
        dock.AddPanel(new HDevDockPanel("flotte", "Flotte",
            new Avalonia.Controls.ScrollViewer { Content = fleet }, "🛩️"), DockRegion.Left);

        // Volet droit : détails de sélection.
        dock.AddPanel(new HDevDockPanel("details", "Détails",
            Pad("Détails de l'appareil / du vol sélectionné.\n\nRedimensionne les volets par les splitters entre eux."),
            "🔍"), DockRegion.Right);

        // Volet bas : journal.
        dock.AddPanel(new HDevDockPanel("journal", "Journal",
            Pad("[08:15] TY201 embarquement\n[08:52] TY201 pushback\n[09:03] TY201 airborne"),
            "📜"), DockRegion.Bottom);

        // Barre d'outils : sérialisation du layout.
        var save = new HDevButton { Text = "Sauver layout", Variant = ButtonVariant.Secondary };
        var load = new HDevButton { Text = "Charger layout", Variant = ButtonVariant.Secondary, IsEnabled = false };
        var status = new Avalonia.Controls.TextBlock
        {
            VerticalAlignment = Avalonia.Layout.VerticalAlignment.Center,
            Foreground = new Avalonia.Media.SolidColorBrush(HDev.UI.Controls.Theme.HDevTheme.Text.Secondary),
    ...
    private Avalonia.Controls.Control BuildHtmlViewDemo()
    {
        ...
        var view = new HDevHtmlView { Html = html };
        view.LinkClicked += async (s, href) =>
            await HDevMessageBox.ShowInfoAsync(this, $"Lien cliqué (l'app décide quoi en faire) :\n{href}", "LinkClicked");
    ...
    {
        var map = new HDevMindMap();
        var root = new MindMapNode("Suite HDev.UI.Controls");
        ...
        toolbar.AddButton("🖼", "Export PNG", async () =>
        {
            var path = System.IO.Path.Combine(
                ...
                "mindmap-export.png");
            map.ExportPng(path);
            await HDevMessageBox.ShowInfoAsync(this, $"Carte exportée :\n{path}", "Export PNG");
        });
        toolbar.AddSeparator();
        ...
        var hint = new Avalonia.Controls.TextBlock
        {
            ...
            Foreground = new Avalonia.Media.SolidColorBrush(
                HDev.UI.Controls.Theme.HDevTheme.Text.Secondary)
        };
    ...
    /// <summary>
    /// Vitrine de HDevRoster : un mois de roster equipage, une voie par journee.
    /// </summary>
    /// <remarks>
    ...
    {
        var roster = new HDevRoster { LaneSpan = RosterEngine.Day };
        ...
        roster.BlockDoubleClicked += async (s, e) =>
            await HDevMessageBox.ShowInfoAsync(this,
    ...
    {
        var scheduler = new HDevScheduler();
        ...
        var monthCalendar = new HDevMonthCalendar
        {
            Margin = new Avalonia.Thickness(8),
    ...
    private Avalonia.Controls.Control BuildInboxDemo()
    {
        ...
        var list = new HDevListView
        {
            ...
            ItemAdapter = item =>
            {
                ...
                return new HDevListItemContent
                {
                    Title = mail.From,
        ...
        list.HoverActions.Add(new HDevListHoverAction("✉", "Marquer non lu", item =>
        {
            ((DemoMail)item).IsRead = false;
            list.Refresh();
        }));
        list.HoverActions.Add(new HDevListHoverAction("⚑", "Drapeau", item =>
        {
            var mail = (DemoMail)item;
            mail.IsFlagged = !mail.IsFlagged;
            list.Refresh();
        }));
        list.HoverActions.Add(new HDevListHoverAction("🗑", "Supprimer", item =>
        {
        ...
        var folders = new HDevTreeView { AllowDropItems = true };
        ...
        folders.ItemDropped += (s, drop) =>
        {
            ...
            {
                mails.Remove(mail);
                HDevToast.Show(this, "Message déplacé",
                    $"« {mail.Subject} » → {drop.Node.Text}", "📁");
            }
        };

        // Recherche à suggestions (filtre la liste)
        var search = new HDevSearchControl
        {
        ...
        simulate.Click += (s, e) =>
        {
            ...
            mails.Insert(0, mail);
            HDevToast.Show(this, "Nouveau message", $"{mail.From} — {mail.Subject}", "📧",
    ...
    private Avalonia.Controls.Control BuildRichEditDemo()
    {
        ...
        toolbar.AddButton("🔴", "Texte rouge", () => edit.SetTextColor(HDev.UI.Controls.Theme.HDevTheme.Accent.Error));
        ...
        toolbar.AddButton("👁", "Aperçu HTML", async () =>
        {
            ...
            var preview = new HDevHtmlView { Html = edit.Html, MinWidth = 500 };
            var window = new HDevWindow
            {
                Title = "Aperçu (HDevHtmlView sur le HTML sérialisé)",
                Width = 560,
                Height = 420,
        ...
        var recipients = new HDevTokenEdit
        {
    ...
    private Avalonia.Controls.Control BuildGanttDemo()
    {
        ...
        var todayButton = new HDevButton { Text = "Aujourd'hui", Variant = ButtonVariant.Secondary, Size = ButtonSize.Small };
        todayButton.Click += (s, e) => gantt.ScrollToToday();

        var criticalToggle = new HDevToggleSwitch { OnLabel = "Chemin critique", OffLabel = "Chemin critique", ShowLabels = true };
        criticalToggle.Toggled += (s, e) => gantt.HighlightCriticalPath = criticalToggle.IsOn;

        var autoToggle = new HDevToggleSwitch { OnLabel = "Ordonnancement auto", OffLabel = "Ordonnancement auto", ShowLabels = true };
        autoToggle.Toggled += (s, e) =>
        {
            if (gantt.Project != null)
                gantt.Project.AutoSchedule = autoToggle.IsOn;
        };
        var stressButton = new HDevButton { Text = "Test de charge (2 500 tâches)", Variant = ButtonVariant.Ghost, Size = ButtonSize.Small };
        ...
        var baselineButton = new HDevButton { Text = "Figer la baseline", Variant = ButtonVariant.Secondary, Size = ButtonSize.Small };
        var baselineToggle = new HDevToggleSwitch { OnLabel = "Baseline", OffLabel = "Baseline", ShowLabels = true };
        ...
        var exportButton = new HDevButton { Text = "Export PNG", Variant = ButtonVariant.Secondary, Size = ButtonSize.Small };
        exportButton.Click += async (s, e) =>
        {
            ...
            await HDevMessageBox.ShowInfoAsync(this, $"Diagramme complet exporté :\n{path}", "Export PNG");
        };

        // Undo / redo (Ctrl+Z / Ctrl+Y aussi, contrôle focalisé)
        var undoButton = new HDevButton { Text = "↶ Annuler", Variant = ButtonVariant.Secondary, Size = ButtonSize.Small, IsEnabled = false };
        var redoButton = new HDevButton { Text = "↷ Rétablir", Variant = ButtonVariant.Secondary, Size = ButtonSize.Small, IsEnabled = false };
        ...
        var saveButton = new HDevButton { Text = "💾 Sauver", Variant = ButtonVariant.Secondary, Size = ButtonSize.Small };
        var loadButton = new HDevButton { Text = "📂 Charger", Variant = ButtonVariant.Secondary, Size = ButtonSize.Small, IsEnabled = false };
    ...
    private void InitializeEditorsDemo()
    {
        ...
        var dateEdit = this.FindControl<HDevDateEdit>("DateEditDemo");
        if (dateEdit != null)
        {
        ...
        var timeEdit = this.FindControl<HDevTimeEdit>("TimeEditDemo");
        if (timeEdit != null)
        {
        ...
        var spinEdit = this.FindControl<HDevSpinEdit>("SpinEditDemo");
        if (spinEdit != null)
        {
            spinEdit.MinValue = 0;
        ...
        var comboEditable = this.FindControl<HDevComboBox>("ComboEditable");
        ...
        var lookUp = this.FindControl<HDevLookUpEdit>("LookUpDemo");
        if (lookUp != null)
        {
            ...
            lookUp.Columns.Add(new HDev.UI.DataGrid.Models.GridColumn
            {
    ...
    {
        var toolbar = this.FindControl<HDevToolbar>("MainToolbar");
        ...
        var darkToggle = toolbar.AddToggle("🌙", "Sombre",
            isChecked: HDev.UI.Controls.Theme.HDevTheme.IsDark);
        darkToggle.Click += (s, e) =>
        {
            HDev.UI.Controls.Theme.HDevTheme.Variant = darkToggle.IsChecked
                ? HDev.UI.Controls.Theme.HDevThemeVariant.Dark
                : HDev.UI.Controls.Theme.HDevThemeVariant.Light;

            // Fond racine de la démo (hors HDevWindow)
            var root = this.FindControl<Avalonia.Controls.Grid>("RootGrid");
            if (root != null)
                root.Background = new Avalonia.Media.SolidColorBrush(
                    HDev.UI.Controls.Theme.HDevTheme.Background.Window);
        };
    }
    ...
    private void InitializeStatusBar()
    {
        ...
        statusBar.AddPanel("UTF-8", HDevStatusBarPanelAlignment.Right);
        statusBar.AddSeparator(HDevStatusBarPanelAlignment.Right);
        var zoomPanel = statusBar.AddPanel("100 %", HDevStatusBarPanelAlignment.Right, icon: "🔍");
    ...
    private void InitializeMessageBoxDemo()
    {
        ...
        void ShowResult(HDevDialogResult r)
        {
            if (result != null) result.Text = $"Résultat : {r}";
        }

        var btnInfo = this.FindControl<HDevButton>("BtnMsgInfo");
        if (btnInfo != null)
            btnInfo.Click += async (s, e) =>
                ShowResult(await HDevMessageBox.ShowInfoAsync(this, "Les contrôles du lot 1 sont installés."));

        var btnWarning = this.FindControl<HDevButton>("BtnMsgWarning");
        if (btnWarning != null)
            btnWarning.Click += async (s, e) =>
                ShowResult(await HDevMessageBox.ShowWarningAsync(this, "Cette action modifiera la configuration."));

        var btnError = this.FindControl<HDevButton>("BtnMsgError");
        if (btnError != null)
            btnError.Click += async (s, e) =>
                ShowResult(await HDevMessageBox.ShowErrorAsync(this, "Impossible de contacter le serveur.\nVérifiez la connexion réseau."));

        var btnQuestion = this.FindControl<HDevButton>("BtnMsgQuestion");
        if (btnQuestion != null)
            btnQuestion.Click += async (s, e) =>
                ShowResult(await HDevMessageBox.ShowQuestionAsync(this, "Voulez-vous enregistrer les modifications avant de quitter ?"));
    }

    private void InitializeDataGrid()
    {
        var dataGrid = this.FindControl<HDev.UI.DataGrid.HDevDataGrid>("EmployeeDataGrid");
        if (dataGrid != null)
        {
            dataGrid.ItemsSource = Employees;
        }

        var exportButton = this.FindControl<HDevButton>("BtnExportCsv");
    ...
    private void InitializeComboBoxDemo()
    {
        ...
        var comboCountries = this.FindControl<HDevComboBox>("ComboCountries");
        ...
        var comboWithSelection = this.FindControl<HDevComboBox>("ComboWithSelection");
        ...
        // ComboBox disabled
        var comboDisabled = this.FindControl<HDevComboBox>("ComboDisabled");
        ...
        var comboReadOnly = this.FindControl<HDevComboBox>("ComboReadOnly");
        ...
        var comboSorted = this.FindControl<HDevComboBox>("ComboSorted");
        if (comboSorted != null)
        {
    ...
    {
        var menuBar = this.FindControl<HDevMenuBar>("MainMenuBar");
        if (menuBar == null) return;
        
        void Notify(string action) =>
            HDevToast.Show(this, action, "Déclenché par le menu ou son raccourci clavier", "⌨️",
        ...
        fileMenu.Items.Add(new HDevMenuItem("New", "📄", "Ctrl+N", () => Notify("Nouveau document")));
        fileMenu.Items.Add(new HDevMenuItem("Open", "📂", "Ctrl+O", () => Notify("Ouvrir")));
        fileMenu.Items.Add(new HDevMenuItem("Save", "💾", "Ctrl+S", () => Notify("Enregistré")));
        fileMenu.Items.Add(new HDevMenuItem("Save As...", null, "Ctrl+Shift+S", () => Notify("Enregistrer sous")));
        fileMenu.Items.Add(HDevMenuItem.Separator());
        fileMenu.Items.Add(new HDevMenuItem("Export", "📤"));
        fileMenu.Items.Add(new HDevMenuItem("Print", "🖨️", "Ctrl+P", () => Notify("Impression")));
        fileMenu.Items.Add(HDevMenuItem.Separator());
        fileMenu.Items.Add(new HDevMenuItem("Exit", null, "Alt+F4", Close));
        menuBar.Items.Add(fileMenu);

        // Edit menu
        var editMenu = new HDevMenuBarItem("Edit");
        editMenu.Items.Add(new HDevMenuItem("Undo", "↩️", "Ctrl+Z", () => Notify("Annuler")));
        editMenu.Items.Add(new HDevMenuItem("Redo", "↪️", "Ctrl+Y", () => Notify("Rétablir")));
        editMenu.Items.Add(HDevMenuItem.Separator());
        editMenu.Items.Add(new HDevMenuItem("Cut", "✂️", "Ctrl+X"));
        editMenu.Items.Add(new HDevMenuItem("Copy", "📋", "Ctrl+C"));
        editMenu.Items.Add(new HDevMenuItem("Paste", "📌", "Ctrl+V"));
        editMenu.Items.Add(HDevMenuItem.Separator());
        editMenu.Items.Add(new HDevMenuItem("Select All", null, "Ctrl+A"));
        menuBar.Items.Add(editMenu);
        
        // View menu
        var viewMenu = new HDevMenuBarItem("View");
        viewMenu.Items.Add(new HDevMenuItem("Zoom In", "🔍", "Ctrl++"));
        viewMenu.Items.Add(new HDevMenuItem("Zoom Out", "🔎", "Ctrl+-"));
        viewMenu.Items.Add(new HDevMenuItem("Reset Zoom", null, "Ctrl+0"));
        viewMenu.Items.Add(HDevMenuItem.Separator());
        viewMenu.Items.Add(new HDevMenuItem { Header = "Show Grid", IsCheckable = true, IsChecked = true });
        viewMenu.Items.Add(new HDevMenuItem { Header = "Show Rulers", IsCheckable = true });
        menuBar.Items.Add(viewMenu);
        
        // Help menu
        var helpMenu = new HDevMenuBarItem("Help");
        helpMenu.Items.Add(new HDevMenuItem("Documentation", "📖", "F1"));
        helpMenu.Items.Add(new HDevMenuItem("Check for Updates", "🔄"));
        helpMenu.Items.Add(HDevMenuItem.Separator());
        helpMenu.Items.Add(new HDevMenuItem("About HDev.UI.Controls", "ℹ️"));
        menuBar.Items.Add(helpMenu);
    }
    
    private void InitializeRibbon()
    {
        var ribbon = this.FindControl<HDevRibbon>("MainRibbon");
        if (ribbon == null) return;
        
        // Home Tab
        var homeTab = new HDevRibbonTab("Home");
        ...
        homeTab.AddGroup("Font", g =>
        {
            ...
            g.DialogLauncher = async () =>
                await HDevMessageBox.ShowInfoAsync(this, "Dialog launcher du groupe Font : ouvrirait la boîte de dialogue complète des polices.", "Font");
        });
        ...
        // Insert Tab
        var insertTab = new HDevRibbonTab("Insert");
        ...
        // View Tab
        var viewTab = new HDevRibbonTab("View");
