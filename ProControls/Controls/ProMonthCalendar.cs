...
/// Remplace le Calendar Fluent (souveraineté) — utilisé par ProDateEdit.
/// </summary>
public class ProMonthCalendar : Control
{
    ...
    static ProMonthCalendar()
    {
        FocusableProperty.OverrideDefaultValue<ProMonthCalendar>(true);
    }

    public ProMonthCalendar()
    {
        Width = 7 * CellSize + 12;
    ...
    public override void Render(DrawingContext context)
    {
        ...
        var title = Text(_displayMonth.ToString("MMMM yyyy", CultureInfo.CurrentCulture),
            ProTheme.Text.Primary, 13, FontWeight.SemiBold);
        ...
        for (int d = 0; d < 7; d++)
        {
            ...
            var label = Text(name, ProTheme.Text.Secondary, 10);
        ...
        for (int w = 0; w < 6; w++)
        {
            for (int d = 0; d < 7; d++)
            {
                ...
                if (isSelected)
                    ...
                    context.DrawEllipse(null, new Pen(new SolidColorBrush(ProTheme.Accent.Primary), 1.4),
                        center, CellSize / 2 - 3, CellSize / 2 - 3);
                else if (isHovered && !isDisabled)
                    context.DrawEllipse(new SolidColorBrush(ProTheme.Background.ControlHover), null,
                        center, CellSize / 2 - 3, CellSize / 2 - 3);

                var color = isSelected ? Colors.White
                    : isDisabled ? ProTheme.Text.Disabled
                    : isCurrentMonth ? ProTheme.Text.Primary
    ...
    private void DrawNavArrow(DrawingContext context, Rect rect, bool hovered, bool left)
    {
        ...
        var pen = new Pen(new SolidColorBrush(ProTheme.Text.Secondary), 1.5);
        var c = rect.Center;
        var dx = left ? 2.0 : -2.0;
