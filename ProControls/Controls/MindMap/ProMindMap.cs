...
/// </summary>
public class ProMindMap : Control
{
    ...
    static ProMindMap()
    {
        FocusableProperty.OverrideDefaultValue<ProMindMap>(true);
        ProTheme.VariantChanged += (s, e) => _themeGeneration++;
    }
    ...
    private Color BranchColorOf(MindMapNode node)
    {
        ...
        if (current.Parent == null)
            return ProTheme.Accent.Primary; // racine
    ...
    private void BeginEdit(MindMapNode node, bool selectAll)
    {
        ...
        _editNode = node;
        _editor = new ProTextBox { Text = node.Text, MinHeight = 0 };
    ...
    public override void Render(DrawingContext context)
    {
        ...
        foreach (var placed in _placed)
        {
            ...
            context.DrawGeometry(null,
                new Pen(new SolidColorBrush(ProTheme.WithOpacity(color, 170)),
            ...
            _placedByNode.TryGetValue(_selected, out var selectedPlaced))
        {
            ...
            if (_selected.Parent != null)
            {
                ...
                var errorColor = ProTheme.Accent.Error;
                context.DrawEllipse(new SolidColorBrush(ProTheme.Background.Panel),
            ...
            _placedByNode.TryGetValue(_dragNode, out var dragPlaced))
        {
            ...
            var color = _dropTarget == null ? ProTheme.Text.Secondary
                : _dropValid ? ProTheme.Accent.Success : ProTheme.Accent.Error;
            context.DrawLine(
    ...
    private void RenderNode(DrawingContext context, MindMapPlacedNode placed, Rect screen)
    {
        ...
        Color fill = isRoot
            ...
            : ProTheme.WithOpacity(color, isSelected ? (byte)70 : isHovered ? (byte)55 : (byte)38);
        var borderPen = isSelected
            ? new Pen(new SolidColorBrush(color), 2.2)
            : new Pen(new SolidColorBrush(ProTheme.WithOpacity(color, 150)), 1.2);
        ...
        if (isRoot)
        {
            var rootText = new FormattedText(node.Text, CultureInfo.CurrentCulture,
                ...
                new Typeface(ProTheme.Typography.FontFamily, weight: FontWeight.SemiBold),
        ...
        if (node.HasChildren)
        {
            ...
            {
                context.DrawEllipse(new SolidColorBrush(ProTheme.Background.Panel),
                    new Pen(new SolidColorBrush(ProTheme.WithOpacity(color, 170)), 1.2),
