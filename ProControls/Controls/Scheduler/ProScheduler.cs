...
/// </summary>
public class ProScheduler : Control
{
    ...
    static ProScheduler()
    {
        FocusableProperty.OverrideDefaultValue<ProScheduler>(true);
    }
    ...
    private void RenderTimeGridView(DrawingContext context)
    {
        ...
        for (int d = 0; d < days; d++)
        {
            ...
            if (isToday)
                context.FillRectangle(new SolidColorBrush(ProTheme.WithOpacity(ProTheme.Accent.Primary, 18)),
                    new Rect(x, 0, colWidth, DayHeaderHeight));

            var header = Text(date.ToString("ddd dd/MM", CultureInfo.CurrentCulture),
                isToday ? ProTheme.Accent.Primary : ProTheme.Text.Primary, 12,
        ...
        {
            var linePen = new Pen(new SolidColorBrush(ProTheme.Border.Subtle), 1);
            var halfPen = new Pen(new SolidColorBrush(ProTheme.WithOpacity(ProTheme.Border.Subtle, 90)), 1);

            // Heures non ouvrées ombrées
            var shade = new SolidColorBrush(ProTheme.WithOpacity(ProTheme.Text.Secondary, 10));
            for (int d = 0; d < days; d++)
            {
                var date = start.AddDays(d);
                var x = TimeGutterWidth + d * colWidth;
            ...
            for (int h = 0; h <= 24; h++)
            {
                ...
                var label = Text($"{h:00}:00", ProTheme.Text.Secondary, 10);
            ...
            if (today >= start && today < rangeEnd)
            {
                ...
                context.DrawLine(new Pen(new SolidColorBrush(ProTheme.Accent.Error), 1.5),
                    new Point(x, y), new Point(x + colWidth, y));
                context.DrawEllipse(new SolidColorBrush(ProTheme.Accent.Error), null,
                    new Point(x + 3, y), 3, 3);
            }
        ...
        double colWidth, double gridBottom)
    {
        ...
        var pen = new Pen(new SolidColorBrush(ProTheme.Accent.Primary), 1.5)
        {
            DashStyle = new DashStyle(new double[] { 3, 3 }, 0)
        };
        context.DrawRectangle(
            new SolidColorBrush(ProTheme.WithOpacity(ProTheme.Accent.Primary, 30)), pen, rect, 4, 4);

        var label = Text($"{_ghostStart:HH:mm} → {_ghostEnd:HH:mm}", ProTheme.Text.Primary, 11);
    ...
    private void RenderMonthView(DrawingContext context)
    {
        ...
        for (int d = 0; d < 7; d++)
        {
            ...
            var header = Text(name, ProTheme.Text.Secondary, 11);
            context.DrawText(header, Crisp.Snap(new Point(
            ...
            for (int d = 0; d < 7; d++)
            {
                ...
                if (!isCurrentMonth)
                    context.FillRectangle(new SolidColorBrush(
                        ProTheme.WithOpacity(ProTheme.Text.Secondary, 8)), cell);

                context.DrawRectangle(null, linePen, cell);

                // Numéro du jour (pastille accent = aujourd'hui)
                var number = Text(date.Day.ToString(),
                    isToday ? Colors.White : isCurrentMonth ? ProTheme.Text.Primary : ProTheme.Text.Secondary,
                    12, isToday ? FontWeight.SemiBold : FontWeight.Regular);
                if (isToday)
                    context.DrawEllipse(new SolidColorBrush(ProTheme.Accent.Primary), null,
                ...
                for (int i = 0; i < dayOccurrences.Count && i < maxChips; i++)
                {
                    ...
                    {
                        var more = Text($"+ {dayOccurrences.Count - i} autres", ProTheme.Text.Secondary, 10);
    ...
    private void RenderEventBlock(DrawingContext context, ScheduleOccurrence occ, Rect rect, bool compact)
    {
        ...
        var color = master.Color ?? ProTheme.Accent.Primary;
        var isSelected = ReferenceEquals(master, _selectedEvent);
        var isHovered = ReferenceEquals(master, _hoveredEvent);

        context.DrawRectangle(
            new SolidColorBrush(ProTheme.WithOpacity(color, isSelected ? (byte)230 : isHovered ? (byte)190 : (byte)160)),
            isSelected ? new Pen(new SolidColorBrush(ProTheme.Text.Primary), 1.5) : null,
            rect, 4, 4);
