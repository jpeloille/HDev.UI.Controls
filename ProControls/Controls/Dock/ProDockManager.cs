...
/// </summary>
public sealed class ProDockPanel
{
    ...
    public ProDockPanel(string id, string title, Control? content = null,
...
/// </summary>
public sealed class ProDockManager : Decorator
{
    private readonly Dictionary<string, ProDockPanel> _panels = new();
    ...
    public event EventHandler<ProDockPanel>? PanelClosed;

    public ProDockManager()
    {
    ...
    public ProDockPanel AddPanel(ProDockPanel panel, DockRegion region)
    {
    ...
    private Control BuildGroup(DockTabGroup group)
    {
        ...
        var host = new Border { Background = new SolidColorBrush(ProTheme.Background.Panel) };
        _hosts.Add(host);
    ...
    private void OnThemeVariantChanged(object? sender, EventArgs e)
    {
        foreach (var splitter in _splitters)
            splitter.Background = new SolidColorBrush(ProTheme.Border.Subtle);
        foreach (var host in _hosts)
            host.Background = new SolidColorBrush(ProTheme.Background.Panel);
...
/// <summary>
/// Bandeau d'onglets d'un groupe docké : reprend l'idiome pilule de ProTabControl
/// mais dédié au docking (sélection, fermeture ×). Le drag-réordonnance viendra
/// en phase 2 — d'où une classe distincte plutôt qu'une greffe sur ProTabControl.
/// </summary>
internal sealed class DockTabStrip : Control
{
    ...
    private double MeasureTabWidth(DockItem item)
        => 10 + CreateText(TabText(item), ProTheme.Text.Primary).Width + 10 + (item.CanClose ? CloseSize + 3 : 0);
