...
**⚑ HTML TipTap = format de stockage (acté le 19/07/2026)** : le HTML `getHTML()` de TipTap (sérialisation fermée d'un document ProseMirror) devient le format de stockage du texte riche applicatif. Vocabulaire figé **storage-v1** : source de vérité `Documents/HtmlVocabulary.cs` + miroir humain `Documents/StorageHtml-v1.md` (toute divergence = bug). Trois listes distinctes : accepté (profil Mail tolérant, inchangé — lecteur de mails + collage) ⊃ strict (storage-v1, `Parse(html, HtmlParseOptions.Storage)` = diagnostics jamais d'exception) ⊇ émis (`HtmlSerializer`, dialecte TipTap : strong/em, `<mark>`, `<pre><code>`, tbody, target/rel sur les liens). Chantier en cours : parseur (`<mark>`, `<p></p>` vides matérialisés, gardes de dépilement de contexte), mode strict, sérialiseur, tests de contrat `StorageHtmlContractTests`.
